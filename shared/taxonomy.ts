...
export const BRANCH_META: Record<Branch, { label: string; blurb: string; meter: MeterKey }> = {
  ...
  aws_foundations: {
    ...
    blurb: 'IAM, VPC and PrivateLink, compute choice, data stores, KMS, observability.',
    meter: 'platform',
  },
  aws_ai_platform: {
    label: 'AWS AI Platform',
    blurb: 'Bedrock, Knowledge Bases, Agents, Guardrails, SageMaker, OpenSearch vectors.',
    meter: 'platform',
  },
  azure_foundations: {
    label: 'Azure Foundations',
    blurb: 'Entra ID, VNets and Private Endpoints, compute choice, data stores, Key Vault.',
    meter: 'platform',
  },
  azure_ai_platform: {
...
export const TAXONOMY: TaxonomyNode[] = [
  ...
  // ── AWS Foundations ──────────────────────────────────────────────────────
  n('aws.iam', 'aws_foundations', 'IAM roles & policies',
    'Principals, assumed roles, policy evaluation, and why wildcards lose security reviews.',
    { cloud: 'aws' }),
  n('aws.identity_federation', 'aws_foundations', 'Federation & IAM Roles Anywhere',
    'OIDC federation from CI and other clouds: the answer to stored access keys.',
    { cloud: 'aws', requires: ['aws.iam'] }),
  n('aws.organizations', 'aws_foundations', 'Organizations & SCPs',
    'Account hierarchy, service control policies, and guardrails that cannot be argued with.',
    { cloud: 'aws' }),
  n('aws.vpc', 'aws_foundations', 'VPC & networking',
    'Subnets, route tables, NAT, security groups versus NACLs, and where the bill hides.',
    { cloud: 'aws' }),
  n('aws.privatelink', 'aws_foundations', 'PrivateLink & endpoints',
    'Interface and gateway endpoints, and publishing a service into a customer VPC.',
    { cloud: 'aws', requires: ['aws.vpc'] }),
  n('aws.transit', 'aws_foundations', 'Transit Gateway & hybrid',
    'Hub and spoke that actually routes, Direct Connect, and peering that does not scale.',
    { cloud: 'aws', requires: ['aws.vpc'] }),
  n('aws.landing_zone', 'aws_foundations', 'Landing zones & Control Tower',
    'Multi-account baselines, guardrails and the account vending machine.',
    { cloud: 'aws' }),
  n('aws.compute_choice', 'aws_foundations', 'Lambda vs ECS vs EKS vs EC2',
    'How much of the platform you want to own, priced in engineer-hours.',
    { cloud: 'aws' }),
  n('aws.eks', 'aws_foundations', 'EKS',
    'Managed Kubernetes, IRSA, node groups, and the operational bill nobody quotes.',
    { cloud: 'aws' }),
  n('aws.s3', 'aws_foundations', 'S3 & storage classes',
    'Consistency, lifecycle, access points, and the request charges that surprise people.',
    { cloud: 'aws' }),
  n('aws.dynamodb', 'aws_foundations', 'DynamoDB',
    'Partition key design, hot partitions, GSIs, and when a relational store was correct.',
    { cloud: 'aws' }),
  n('aws.rds_aurora', 'aws_foundations', 'RDS & Aurora',
    'Managed Postgres and MySQL, failover behavior, read replicas and Serverless v2.',
    { cloud: 'aws' }),
  n('aws.redshift', 'aws_foundations', 'Redshift & Athena',
    'Warehouse versus query-in-place, and which one the customer actually needs.',
    { cloud: 'aws' }),
  n('aws.streaming', 'aws_foundations', 'SQS, SNS, EventBridge & Kinesis',
    'Queues, fan-out, event buses and streams, and picking the right one first time.',
    { cloud: 'aws' }),
  n('aws.kms', 'aws_foundations', 'KMS & encryption',
    'Customer managed keys, grants, key policies and the revocation story.',
    { cloud: 'aws' }),
  n('aws.observability', 'aws_foundations', 'CloudWatch & X-Ray',
    'Metrics, logs, traces, and the cardinality that quietly costs more than compute.',
    { cloud: 'aws' }),
  n('aws.billing', 'aws_foundations', 'Billing, quotas & Savings Plans',
    'Reserved capacity, Savings Plans, service quotas and cost allocation tags.',
    { cloud: 'aws' }),

  // ── AWS AI Platform ──────────────────────────────────────────────────────
  n('aws.bedrock', 'aws_ai_platform', 'Bedrock',
    'Managed access to third-party models, provisioned throughput and model choice.',
    { cloud: 'aws' }),
  n('aws.bedrock_agents', 'aws_ai_platform', 'Bedrock Agents & action groups',
    'Orchestration, tool invocation and session state on the managed path.',
    { cloud: 'aws', requires: ['aws.bedrock'] }),
  n('aws.knowledge_bases', 'aws_ai_platform', 'Knowledge Bases & RAG',
    'Managed ingestion, chunking and retrieval, and where the abstraction runs out.',
    { cloud: 'aws', requires: ['aws.bedrock'] }),
  n('aws.guardrails', 'aws_ai_platform', 'Bedrock Guardrails',
    'Content filters, denied topics, PII redaction and grounding checks.',
    { cloud: 'aws', requires: ['aws.bedrock'] }),
  n('aws.opensearch_vectors', 'aws_ai_platform', 'OpenSearch & vector search',
    'Vector indexes, hybrid retrieval and the recall knobs that fail silently.',
    { cloud: 'aws' }),
  n('aws.sagemaker', 'aws_ai_platform', 'SageMaker',
    'Training, endpoints, and when a managed model API was the cheaper answer.',
    { cloud: 'aws' }),
  n('aws.bedrock_residency', 'aws_ai_platform', 'Model regions & data handling',
    'Where inference happens, cross-region inference, and what the DPA actually says.',
    { cloud: 'aws', requires: ['aws.bedrock'] }),

  // ── Azure Foundations ────────────────────────────────────────────────────
  n('az.entra', 'azure_foundations', 'Entra ID & RBAC',
    'Tenants, app registrations, role assignments and the scope hierarchy.',
    { cloud: 'azure' }),
  n('az.managed_identity', 'azure_foundations', 'Managed identity',
    'System and user assigned identities: the answer to secrets in app settings.',
    { cloud: 'azure', requires: ['az.entra'] }),
  n('az.management_groups', 'azure_foundations', 'Management groups & Policy',
    'Subscription hierarchy, Azure Policy, initiatives and deny effects.',
    { cloud: 'azure' }),
  n('az.vnet', 'azure_foundations', 'VNets & NSGs',
    'Subnets, network security groups, service endpoints and hub-spoke topology.',
    { cloud: 'azure' }),
  n('az.private_endpoint', 'azure_foundations', 'Private Endpoints & Private Link',
    'Private IPs for PaaS, the DNS that breaks it, and publishing to a customer tenant.',
    { cloud: 'azure', requires: ['az.vnet'] }),
  n('az.landing_zone', 'azure_foundations', 'Landing zones',
    'Cloud Adoption Framework baselines, platform versus application subscriptions.',
    { cloud: 'azure' }),
  n('az.compute_choice', 'azure_foundations', 'Functions vs App Service vs ACA vs AKS',
    'Container Apps, App Service and AKS, chosen by how much platform you want to own.',
    { cloud: 'azure' }),
  n('az.aks', 'azure_foundations', 'AKS',
    'Managed Kubernetes, workload identity, node pools and the upgrade treadmill.',
    { cloud: 'azure' }),
  n('az.storage', 'azure_foundations', 'Storage & Data Lake',
    'Blob tiers, ADLS Gen2 hierarchical namespace, SAS tokens and lifecycle rules.',
    { cloud: 'azure' }),
  n('az.cosmos', 'azure_foundations', 'Cosmos DB',
    'Partition keys, request units, consistency levels and the cost of getting them wrong.',
    { cloud: 'azure' }),
  n('az.sql', 'azure_foundations', 'Azure SQL & PostgreSQL',
    'Managed Instance versus single database, flexible server, and failover groups.',
    { cloud: 'azure' }),
  n('az.synapse_fabric', 'azure_foundations', 'Synapse & Fabric',
    'Warehouse, lakehouse and the analytics story customers are being sold.',
    { cloud: 'azure' }),
  n('az.messaging', 'azure_foundations', 'Service Bus & Event Hubs',
    'Queues and topics versus event streams, sessions, and dead-letter handling.',
    { cloud: 'azure' }),
  n('az.key_vault', 'azure_foundations', 'Key Vault & encryption',
    'Customer managed keys, secrets, certificates and the access model.',
    { cloud: 'azure' }),
  n('az.monitor', 'azure_foundations', 'Azure Monitor & App Insights',
    'Log Analytics workspaces, KQL, distributed tracing and ingestion cost.',
    { cloud: 'azure' }),
  n('az.billing', 'azure_foundations', 'Billing, quotas & reservations',
    'Reservations, savings plans, quota requests and cost management.',
    { cloud: 'azure' }),
  ...
  n('az.openai', 'azure_ai_platform', 'Azure OpenAI',
    ...
    { cloud: 'azure' }),
  n('az.ai_search', 'azure_ai_platform', 'AI Search & vectors',
    'Hybrid retrieval, semantic ranker, integrated vectorization and index design.',
    { cloud: 'azure' }),
  n('az.content_safety', 'azure_ai_platform', 'Content Safety & guardrails',
    'Content filters, prompt shields, groundedness detection and severity thresholds.',
    { cloud: 'azure' }),
  n('az.ai_agents', 'azure_ai_platform', 'Agent Service & tools',
    'Managed agent runtime, tool calling, threads and identity for agent actions.',
    { cloud: 'azure', requires: ['az.ai_foundry'] }),
  n('az.ai_residency', 'azure_ai_platform', 'Data residency & processing',
    'Data zones, regional deployments, abuse monitoring opt-out and what the DPA covers.',
    { cloud: 'azure' }),

  // ── Cross-cloud ──────────────────────────────────────────────────────────
  n('xcloud.mapping', 'gcp_foundations', 'Cross-cloud mapping',
    'PSC, PrivateLink and Private Endpoint, and friends. The translation layer.',
    { cloud: 'neutral' }),
];
