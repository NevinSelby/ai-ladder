...
export const DRILL_EXPANSION: DrillItem[] = [
  // ── GCP AI Platform ──────────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Memory Bank: long-term facts persisted across sessions' },
        { id: 'b', text: 'Sessions: keep one session open per user forever', whyWrong: 'A session is one conversation’s event history. Keeping it open forever bloats context and cost, and one crash loses everything.' },
        { id: 'c', text: 'Re-send the full transcript of every past conversation in the prompt', whyWrong: 'Context windows and token budgets make this collapse within weeks of real usage.' },
        { id: 'd', text: 'Fine-tune the model on each user’s history', whyWrong: 'Fine-tuning bakes knowledge into weights: per-user tuning is slow, expensive and unmaintainable at any scale.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Run the ADK eval sets: recorded cases comparing tool trajectories and final responses' },
        { id: 'b', text: 'Manually chat with the agent for ten minutes', whyWrong: 'Manual poking finds the failures you already suspected. The regression that costs you a customer is the one you did not think to try.' },
        { id: 'c', text: 'Diff the prompt text and reason about the change', whyWrong: 'Prompt diffs do not predict behavior. Small wording changes reroute tool calls in ways reading cannot catch.' },
        { id: 'd', text: 'Rely on type checking to catch behavioral changes', whyWrong: 'Types verify the code compiles, not that the model still picks the right tool.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A2A: cross-vendor agent-to-agent discovery and task exchange' },
        { id: 'b', text: 'MCP', whyWrong: 'MCP standardizes an agent’s connection to tools and data sources, not delegation between two autonomous agents.' },
        { id: 'c', text: 'A shared REST API with an OpenAPI spec', whyWrong: 'Workable plumbing, but every pair of teams reinvents auth, task lifecycle and streaming. The protocol exists to stop that.' },
        { id: 'd', text: 'Both agents writing to a shared database table', whyWrong: 'Integration by shared table couples release cycles and has no contract for task state, errors or capability discovery.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Model Armor as a model-independent screening layer over both' },
        { id: 'b', text: 'Each team writes safety instructions into their system prompt', whyWrong: 'System-prompt guardrails are suggestions to the model, not enforcement, and prompt injection targets exactly this layer.' },
        { id: 'c', text: 'Rely on the models’ built-in safety filters', whyWrong: 'Built-in filters differ per model, are not centrally configurable, and give you no shared audit trail.' },
        { id: 'd', text: 'A regex blocklist in the API gateway', whyWrong: 'Regex catches yesterday’s known-bad strings. Injection attacks are paraphrased past it in minutes.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'When the retrieval strategy itself is the product: custom chunkers, rerankers or filters the managed path cannot express' },
        { id: 'b', text: 'As soon as the corpus exceeds a few thousand documents', whyWrong: 'Corpus size is what managed services handle well. Scale alone is an argument for managed, not against it.' },
        { id: 'c', text: 'Whenever the customer is cost-sensitive', whyWrong: 'Self-assembled pipelines carry engineering and operations cost that usually exceeds the managed premium at moderate scale.' },
        { id: 'd', text: 'Never: managed RAG covers every case', whyWrong: 'Every managed abstraction has an edge. Pretending otherwise is how demos succeed and deployments fail.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'ANN recall settings: the approximate search may not be surfacing that vector as a candidate' },
        { id: 'b', text: 'Whether the LLM is hallucinating the miss', whyWrong: 'Check retrieval before generation: if the chunk never reached the prompt, no amount of prompting fixes it.' },
        { id: 'c', text: 'Network latency between the app and the index', whyWrong: 'Latency produces slow answers or timeouts, not silently absent documents.' },
        { id: 'd', text: 'The size of the context window', whyWrong: 'Possible later in the chain, but "sometimes misses with no errors" is the classic ANN recall signature; verify candidates first.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Serve Claude through Model Garden: GCP billing, GCP IAM, existing vendor relationship' },
        { id: 'b', text: 'Open a separate direct contract with the model provider', whyWrong: 'Correct eventually perhaps, but a new vendor onboarding cycle is months of legal and security review the project timeline does not have.' },
        { id: 'c', text: 'Use a personal API key until procurement catches up', whyWrong: 'Shadow IT with customer data is a security-review failure and potentially a contract breach. Never propose it.' },
        { id: 'd', text: 'Tell them Gemini is the only option on GCP', whyWrong: 'False, and the customer will find out it is false, which costs you the credibility the account runs on.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Document AI processor for extraction, route low-confidence fields to a review queue' },
        { id: 'b', text: 'Send each page image to a multimodal LLM and trust the JSON it returns', whyWrong: 'No per-field confidence signal means you either review everything or review nothing. Both fail at 40k/month.' },
        { id: 'c', text: 'Manual data entry with the LLM double-checking', whyWrong: 'Keeps the full manual cost and adds an AI bill on top. Backwards.' },
        { id: 'd', text: 'OCR to raw text, then regex the fields out', whyWrong: 'Layout is the information in a form. Flattening to raw text destroys the structure the fields live in.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── AI engineering ───────────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The golden set has drifted from real traffic: it no longer contains the cases users actually hit' },
        { id: 'b', text: 'The model got worse while the eval stayed accurate', whyWrong: 'Possible, but a static pass rate with rising complaints points at the measure, not the model. A degrading model usually moves the score.' },
        { id: 'c', text: 'Users are wrong about the quality', whyWrong: 'Sometimes individually, never in aggregate over a trend. Treating complaint volume as noise is how products die confident.' },
        { id: 'd', text: 'The eval threshold is too strict', whyWrong: 'A too-strict threshold produces false alarms, the opposite symptom of passing while quality falls.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Business-rule validation after schema validation: shape-valid is not value-valid' },
        { id: 'b', text: 'A stricter JSON schema', whyWrong: 'You can encode minimum: 0, but the general lesson stands: schemas cannot express most business invariants, like totals matching line items.' },
        { id: 'c', text: 'A bigger model', whyWrong: 'Reduces error frequency, cannot eliminate it. Pipelines are safe because of gates, not because of optimism about the model.' },
        { id: 'd', text: 'Lower temperature', whyWrong: 'Less sampling variance does not stop a model from confidently misreading a credit note as an invoice.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Refuse the admin token: mint scoped credentials per tool, read-only where possible' },
        { id: 'b', text: 'Use the admin token but tell the model not to do destructive things', whyWrong: 'Instructions are not authorization. Prompt injection exists precisely to make the model ignore that sentence.' },
        { id: 'c', text: 'Use the admin token but log every call', whyWrong: 'An audit trail tells you which tickets were deleted. Prevention and detection are not substitutes.' },
        { id: 'd', text: 'Block the integration entirely', whyWrong: 'Unnecessary: scoped credentials make this integration perfectly shippable. Blanket refusal is judgment failure in the other direction.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Structure the prompt for caching: stable prefix first, per-request content last' },
        { id: 'b', text: 'Switch every call to the smallest available model', whyWrong: 'Changes quality everywhere to fix a cost problem that caching removes without touching quality.' },
        { id: 'c', text: 'Shorten the system prompt by cutting tool descriptions', whyWrong: 'Degrading tool selection to save input tokens trades your success rate for pennies caching would have saved anyway.' },
        { id: 'd', text: 'Batch user requests into one giant call', whyWrong: 'Cross-user batching wrecks latency, isolation and error handling. Wrong tool for an input-token problem.' },
      ],
      correctId: 'a',
    },
  },
