...
export const DRILL_GCP_CORE: DrillItem[] = [
  // ── IAM and service accounts ─────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Grant their group Service Account Token Creator on that account and have them impersonate it from gcloud and the client libraries' },
        { id: 'b', text: 'Issue the key but store it in Secret Manager and rotate it every 90 days', whyWrong: 'The key still exists and still ends up cached on a laptop. Rotation shortens the window of a leak, it does not remove the leak.' },
        { id: 'c', text: 'Give each analyst their own user account the same roles as the pipeline service account', whyWrong: 'Now humans hold production data-plane permissions permanently, and the audit trail no longer distinguishes pipeline activity from ad hoc analysis.' },
        { id: 'd', text: 'Run everything through a shared jump host that holds the key', whyWrong: 'A shared host is a shared identity: you lose per-person attribution and you have built a single box everyone needs access to.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'An IAM deny policy on the folder denying iam.serviceAccounts.getAccessToken, with the break-glass group as an exception principal' },
        { id: 'b', text: 'Remove the Token Creator binding and monitor for it being re-added', whyWrong: 'Detective, not preventive. A project owner can re-add the binding and use it before anyone reads the alert.' },
        { id: 'c', text: 'An org policy constraint on service account usage', whyWrong: 'Org policies govern how resources may be configured, not which principals may exercise a permission. There is no constraint that expresses this.' },
        { id: 'd', text: 'A custom role that omits the permission', whyWrong: 'Custom roles only shape one role. Nothing stops someone from also being granted a predefined role that includes the permission.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The Dataflow service agent needs Cloud KMS CryptoKey Encrypter/Decrypter on the key: the managed service acts under its own identity, not the job’s' },
        { id: 'b', text: 'The key is in the wrong key ring', whyWrong: 'A key ring is an organizational container. Wrong ring shows up as a not-found error on the key name, not a permission denial.' },
        { id: 'c', text: 'CMEK is not supported for Dataflow so the setting is silently ignored', whyWrong: 'It is supported, and unsupported settings do not produce permission errors on the key resource.' },
        { id: 'd', text: 'The developer needs the Owner role on the KMS project', whyWrong: 'Handing out Owner to fix a service agent binding is the reflex you want to break. The human is not the caller here.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'It is often unnecessary: roles can be bound directly to the federated principal or principalSet, and the service account is kept only where a service requires that identity type' },
        { id: 'b', text: 'A service account is mandatory: federated identities cannot appear in an IAM allow policy', whyWrong: 'They can. principal:// and principalSet:// members are first-class in allow policies, which is the whole point of direct resource access.' },
        { id: 'c', text: 'The service account is what makes the token short-lived', whyWrong: 'Token lifetime comes from the STS exchange, not from the service account. Direct access tokens are equally short-lived.' },
        { id: 'd', text: 'Only service accounts can be constrained by IAM conditions', whyWrong: 'Conditions attach to the binding, not to the member type, and apply just as well to a federated principal.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'No new tokens can be minted, but any already-issued access token stays valid until it expires, so containment is bounded by the token lifetime rather than instant' },
        { id: 'b', text: 'All outstanding tokens are revoked the moment the provider is deleted', whyWrong: 'Access tokens are bearer credentials validated without a callback to the pool. Deleting the provider stops issuance, not use.' },
        { id: 'c', text: 'Nothing changes until the next attribute mapping refresh', whyWrong: 'There is no refresh cycle to wait for. The exchange fails on the next attempt.' },
        { id: 'd', text: 'The workload silently falls back to the underlying service account credentials', whyWrong: 'There is no fallback path. Federation either produces a token or returns an error.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Workforce Identity Federation' },
        { id: 'b', text: 'Workload Identity Federation', whyWrong: 'That path federates machine workloads. It has no console sign-in flow for a human user.' },
        { id: 'c', text: 'Directory sync from Okta into Cloud Identity', whyWrong: 'This works but provisions exactly the per-user Cloud Identity accounts the customer said they did not want to manage.' },
  ...
  // ── Resource hierarchy and org policy ────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Existing resources keep running untouched: the constraint blocks new creates and updates, so you still need an inventory sweep and a migration plan' },
        { id: 'b', text: 'The out-of-region resources are automatically deleted at the next policy evaluation', whyWrong: 'Org policy has no delete action. If it did, the first accidental scope error would be an outage.' },
        { id: 'c', text: 'They are migrated to the nearest allowed region automatically', whyWrong: 'There is no automatic relocation. Region is baked into most resources at creation.' },
        { id: 'd', text: 'The constraint fails to apply because the folder is already in violation', whyWrong: 'It applies immediately. Existing state does not block the policy from taking effect on future operations.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A custom org policy constraint with a CEL condition on the resource, enforced at the folder' },
        { id: 'b', text: 'A scheduled Cloud Run job that deletes services missing the label', whyWrong: 'Detective and destructive. It races real deployments and turns a metadata gap into a production outage.' },
        { id: 'c', text: 'An IAM condition that only permits deploys carrying the label', whyWrong: 'IAM conditions evaluate request attributes like resource name and time, not arbitrary fields in a resource body.' },
        { id: 'd', text: 'A Security Command Center custom module that raises a finding', whyWrong: 'That gets you a report, and reports do not stop the deploy. Use it as a backstop after the constraint, not instead of it.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The project sets its own policy for that constraint with inheritFromParent false, replacing rather than narrowing the inherited allowed values' },
        { id: 'b', text: 'List constraints always take the union of all levels', whyWrong: 'Inherited list constraints intersect while inheritance is on. Union semantics would make any org-level restriction meaningless.' },
        { id: 'c', text: 'The resource predates the constraint', whyWrong: 'Possible in general, but the question says it is running in a region the policy forbids and the policy is what you are auditing. Check effective policy first.' },
        { id: 'd', text: 'Project-level policies cannot override org-level ones, so this must be a console display bug', whyWrong: 'They can, unless the org-level policy is enforced in a way that denies the override. That assumption is exactly the trap.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  // ── VPC and networking ───────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'None beyond firewall rules: a VPC spans regions and subnet routes are created automatically' },
        { id: 'b', text: 'A VPC peering connection between the two regions', whyWrong: 'Peering joins two separate VPC networks. There is only one network here, so there is nothing to peer.' },
        { id: 'c', text: 'A Cloud VPN tunnel between the regions', whyWrong: 'That would push internal traffic through a gateway and cap it at tunnel throughput, for no benefit.' },
        { id: 'd', text: 'A Cloud Router advertising each subnet to the other region', whyWrong: 'Cloud Router handles dynamic routing to external networks over VPN or Interconnect, not routing inside one VPC.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Enable Private Google Access on the subnet, and point DNS at the appropriate Google API endpoint' },
        { id: 'b', text: 'Deploy Cloud NAT so the VMs can egress to the API endpoint', whyWrong: 'It works, but you are now paying for NAT and routing Google-bound traffic through the internet edge, which undermines a private-access design.' },
        { id: 'c', text: 'Assign ephemeral external IPs to the VMs', whyWrong: 'This directly contradicts the no-external-IP posture, and usually an org policy forbids it anyway.' },
        { id: 'd', text: 'Add an egress firewall rule allowing 443 to the internet', whyWrong: 'Firewall rules permit traffic that already has a route. With no external IP and no NAT there is no route to permit.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Repoint private DNS at the restricted endpoint, which resolves only VPC Service Controls supported services and therefore blocks the rest' },
        { id: 'b', text: 'Add an egress rule to the perimeter for that API', whyWrong: 'Egress rules widen a perimeter. You are trying to close a path, not authorize it.' },
        { id: 'c', text: 'Disable Private Google Access on the subnet', whyWrong: 'That breaks every legitimate API call from the subnet to fix one unwanted one.' },
        { id: 'd', text: 'Block the API with an egress firewall rule on the VM’s tag', whyWrong: 'Both endpoints live in Google-owned ranges shared across APIs, so a firewall rule cannot separate one API from another.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A hierarchical firewall policy at the folder that denies the traffic, since it is evaluated ahead of VPC-level rules' },
        { id: 'b', text: 'A VPC firewall rule at priority 0 denying the same traffic', whyWrong: 'It lives in the same policy the team administers, so whoever can add the allow can also delete or outrank your deny.' },
        { id: 'c', text: 'Remove the team’s compute.securityAdmin role', whyWrong: 'That takes away all rule management, which is exactly the outcome the question rules out.' },
        { id: 'd', text: 'An org policy constraint forbidding open firewall rules', whyWrong: 'Org policy governs resource configuration in supported services, and there is no constraint that expresses arbitrary firewall rule shapes.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Pods draw from a secondary range and each node reserves a block of pod addresses, so the pod range exhausts far faster than the node count suggests' },
        { id: 'b', text: 'The cluster needs a larger primary range for the nodes themselves', whyWrong: 'Nodes take one address each. A /24 holds plenty of nodes: the pressure is on the pod range.' },
        { id: 'c', text: 'Services need public IPs that the subnet cannot supply', whyWrong: 'Kubernetes Services use the cluster’s service range or a load balancer address, not subnet addresses per service.' },
        { id: 'd', text: 'GKE requires a dedicated VPC per cluster', whyWrong: 'Many clusters share a VPC routinely. The constraint is address space, not network count.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The producer-side NAT subnet reserved for the service attachment: consumer connections consume addresses from it and it can exhaust' },
        { id: 'b', text: 'The consumer’s firewall rules', whyWrong: 'Possible for one consumer, but a failure that hits every new consumer while existing ones stay healthy points at shared producer-side state.' },
        { id: 'c', text: 'The load balancer’s backend health checks', whyWrong: 'Unhealthy backends would break existing consumers too. The symptom is specifically about new connections.' },
        { id: 'd', text: 'DNS propagation for the endpoint name', whyWrong: 'A DNS problem produces a name resolution failure on the consumer side, and it would not correlate with the number of consumers already attached.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The consumer publishes their webhook receiver as a service attachment and you attach an endpoint to it: PSC connections are one-directional' },
        { id: 'b', text: 'Reuse the existing endpoint in reverse', whyWrong: 'The connection has a fixed direction. There is no reverse channel on a consumer endpoint.' },
        { id: 'c', text: 'Have the producer call the consumer over the public internet with mutual TLS', whyWrong: 'It works, but you just reintroduced the public path the customer adopted PSC to remove, and their egress review will bounce it.' },
        { id: 'd', text: 'Set up VPC peering between producer and consumer for the callback', whyWrong: 'Peering exposes both address spaces to each other and requires non-overlapping ranges across two organizations, which is precisely what PSC was designed to avoid.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Move to PSC-based instances so each consumer VPC gets its own endpoint, sidestepping both peering transitivity and the shared allocated range' },
        { id: 'b', text: 'Peer the analytics VPC to the service producer network directly', whyWrong: 'You do not own or control the producer network, and peerings to it are managed by the service, not by you.' },
        { id: 'c', text: 'Enlarge the allocated range and add a custom route export', whyWrong: 'Range size was only half the problem. Peering still does not forward traffic from a network peered to your peer.' },
        { id: 'd', text: 'Put a TCP proxy in the original VPC for the analytics VPC to use', whyWrong: 'A workable stopgap, but you now own a proxy fleet, its availability and its audit story, to avoid a supported connectivity model.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'It blocks API access from unauthorized contexts and identities, but data still moves freely between projects inside the perimeter and it inspects no content' },
        { id: 'b', text: 'Correct, provided every service in use is inside the perimeter', whyWrong: 'Even then it is not true. Copying between two in-perimeter projects is unrestricted, and that is a real insider path.' },
        { id: 'c', text: 'Correct, because the perimeter also encrypts data in transit', whyWrong: 'Transit encryption is unrelated and already happens. Attributing it to the perimeter muddles the control story.' },
        { id: 'd', text: 'Wrong, because perimeters only apply to Cloud Storage', whyWrong: 'They cover a broad and growing list of services. The real caveat is that the list is not universal, not that it is one service.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'An ingress rule scoped to the deploy service account and the specific services it touches, with an access level matching the runner’s context' },
        { id: 'b', text: 'Add an access level allowing the CI provider’s published IP ranges', whyWrong: 'Those ranges are shared by every customer of that CI provider, so you have admitted a very large population, not your pipeline.' },
        { id: 'c', text: 'Remove the state bucket’s project from the perimeter', whyWrong: 'Terraform state describes the entire estate. It is one of the last things you want outside the perimeter.' },
        { id: 'd', text: 'Run the pipeline with a user account that has an exemption', whyWrong: 'Automation running as a human breaks attribution and leaves the pipeline dead whenever that person changes roles.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  // ── Hybrid connectivity ──────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'It does not: peering reaches Google public services only, so they still need Dedicated or Partner Interconnect, or Cloud VPN, for internal addresses' },
        { id: 'b', text: 'It does, once they enable Private Google Access on the subnets', whyWrong: 'Private Google Access is about VMs reaching Google APIs outbound. It does not make VPC internal addresses reachable from on-prem.' },
        { id: 'c', text: 'It does, after adding a Cloud Router to advertise the subnets', whyWrong: 'Cloud Router runs BGP over VPN or Interconnect attachments. There is nothing for it to attach to on a peering link.' },
        { id: 'd', text: 'It does for the same region only', whyWrong: 'Regionality is not the issue. Peering simply does not carry traffic to RFC 1918 addresses inside a VPC.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'None: an availability commitment requires redundant links in separate edge availability domains, and the highest tier additionally requires separate metros' },
        ...
        { id: 'c', text: 'It depends on whether they also run a Cloud VPN failover', whyWrong: 'A VPN backup is genuinely good practice for graceful degradation, but it does not confer the Interconnect availability commitment.' },
        { id: 'd', text: 'The commitment applies once the VLAN attachments are in two different regions', whyWrong: 'Attachment region is not the redundancy axis. Edge availability domain, and then metro, is.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Move to Interconnect: per-tunnel VPN throughput is capped, so this only works as many parallel tunnels with ECMP plus on-prem crypto capacity' },
        { id: 'b', text: 'Add a second HA VPN gateway and split traffic by application', whyWrong: 'Manual traffic splitting is brittle and still leaves you scaling tunnel count as volume grows. It postpones the decision.' },
        { id: 'c', text: 'Keep HA VPN and raise the MTU', whyWrong: 'MTU tuning shaves overhead at the margin. It does not move a link past its throughput ceiling by an order of magnitude.' },
        { id: 'd', text: 'Push the migration over the public internet with signed URLs and keep VPN for steady state', whyWrong: 'Defensible for a one-off bulk load, but the question includes ongoing replication, and the customer would still be capped afterward.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Add a custom route advertisement on the Cloud Router for the allocated range, and export custom routes on the peering' },
        { id: 'b', text: 'Add a firewall rule allowing the on-prem range to the database port', whyWrong: 'Firewall rules only matter once packets have a route. A timeout with no route is not a firewall symptom.' },
        { id: 'c', text: 'Give the instance a public IP and restrict by authorized networks', whyWrong: 'It restores connectivity by abandoning the private-only posture the Interconnect exists to provide.' },
        { id: 'd', text: 'Recreate the Interconnect attachment in the same region as the instance', whyWrong: 'Attachment region affects path and cost, not whether a peered range is advertised at all.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Use it as the reference, then keep only the modules matching real requirements and delete the rest before go-live, while the team can still explain every file' },
        { id: 'b', text: 'Fork it unchanged so future upgrades are a clean merge', whyWrong: 'Nobody keeps a foundation unchanged past month two, and unread code you cannot modify is worse than less code you understand.' },
        { id: 'c', text: 'Write everything from scratch so the team owns every line', whyWrong: 'This spends months rediscovering the hierarchy, logging and network patterns the blueprint already got right.' },
        { id: 'd', text: 'Use the blueprint for networking only and click-ops the rest', whyWrong: 'Split ownership between code and console is how drift starts, and the console half is the half nobody can reproduce after an incident.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'An aggregated sink at the org or folder with child inclusion, writing to a destination in a project the workload teams do not administer' },
        { id: 'b', text: 'A log sink in each project routing to a central bucket', whyWrong: 'The sink lives in the project, so the same admin can delete or filter it. The destination being central does not help.' },
        { id: 'c', text: 'Longer retention on the _Default log bucket', whyWrong: 'Retention preserves what arrives. It does nothing about someone stopping the arrival or excluding entries.' },
        { id: 'd', text: 'Removing the Logging Admin role from project admins', whyWrong: 'Reasonable hygiene, but Owner and several other roles still carry the permissions, so it is not a guarantee.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'They share the Cloud Run platform, so decide on packaging and concurrency: a source-deployed handler versus a container you build and control' },
        { id: 'b', text: 'Functions for events, Cloud Run for HTTP', whyWrong: 'Cloud Run handles events through Eventarc and Pub/Sub push perfectly well. The trigger type is not the dividing line.' },
        { id: 'c', text: 'Functions is cheaper because it scales to zero and Cloud Run does not', whyWrong: 'Cloud Run scales to zero by default. Both bill on what they run.' },
        { id: 'd', text: 'Functions for anything under a few hundred lines of code', whyWrong: 'Line count is not an architectural property. Concurrency, dependency control and build reproducibility are.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Move the fan-out state to a shared store such as Memorystore, since any autoscaled instance can disappear, and keep GKE for the case where state truly cannot be externalized' },
        { id: 'b', text: 'Agree and move to GKE, since Kubernetes pods keep memory across restarts', whyWrong: 'They do not. A rescheduled pod loses memory exactly like a recycled Cloud Run instance.' },
        { id: 'c', text: 'Use Cloud Run with session affinity so each client always reaches the same instance', whyWrong: 'Affinity is best-effort and does not survive instance replacement, so it hides the problem until a deploy.' },
        { id: 'd', text: 'Pin the service to one instance with min and max both set to one', whyWrong: 'You have built a single point of failure with no horizontal headroom, and it still restarts on deploy.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  // ── GKE ──────────────────────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'VPA is changing the CPU request that HPA measures utilization against, so each controller keeps invalidating the other’s input' },
        { id: 'b', text: 'The cluster autoscaler is thrashing node pools', whyWrong: 'Node churn follows pod count. Here the pod count itself is unstable, so the cause is above the node layer.' },
        { id: 'c', text: 'The HPA stabilization window is too long', whyWrong: 'A longer window damps oscillation. The problem is a feedback loop between two controllers, not the damping setting.' },
        { id: 'd', text: 'CPU limits are missing so throttling is inconsistent', whyWrong: 'Missing limits affect throttling behavior, not the arithmetic HPA uses, which is measured usage over the request.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Deploy through the Connect gateway, or move the runner inside the VPC, rather than exposing the control plane endpoint' },
        { id: 'b', text: 'Add the CI provider’s published egress ranges to control plane authorized networks', whyWrong: 'Those ranges are shared across that provider’s entire customer base, so you have authorized a very large and anonymous population.' },
        ...
        { id: 'd', text: 'Grant the CI service account cluster-admin so it can use the internal endpoint', whyWrong: 'Permissions do not create network reachability. The call still cannot reach a private address.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Autopilot restricts privileged and host-level access by design, so either use a vendor build that supports Autopilot or run those clusters in Standard' },
        { id: 'b', text: 'Grant the DaemonSet a permissive PodSecurity label on its namespace', whyWrong: 'Autopilot enforces its restrictions at the platform level. A namespace label does not unlock host access.' },
        { id: 'c', text: 'Run the agent as a sidecar in every pod instead', whyWrong: 'A sidecar cannot see host-level activity, so it does not deliver the control the mandate asks for.' },
        { id: 'd', text: 'Use a node pool with a custom node image', whyWrong: 'Autopilot does not expose node pools or node images for you to customize. That is the trade you accepted.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Gateway API: the platform team owns the Gateway, app teams own HTTPRoutes that attach to it' },
        { id: 'b', text: 'Ingress with per-team namespaces and RBAC', whyWrong: 'Ingress bundles routing and load balancer configuration in one object, so an app team that can edit routes can usually edit the rest.' },
        { id: 'c', text: 'One Ingress and load balancer per team', whyWrong: 'It gets you isolation at the cost of an address, a certificate and a bill per team, plus a fragmented edge.' },
        { id: 'd', text: 'A service mesh with per-namespace VirtualServices', whyWrong: 'A mesh governs east-west traffic policy. It does not by itself solve who may configure the external entry point.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A taint on the GPU node pool with a matching toleration on GPU workloads, so ordinary pods are repelled' },
        { id: 'b', text: 'A node selector on the GPU workloads', whyWrong: 'A selector attracts GPU pods to those nodes but places no restriction on everything else, which is the actual problem.' },
        { id: 'c', text: 'Resource limits on the web pods', whyWrong: 'Limits bound consumption on whichever node the pod lands on. They do not influence placement.' },
        { id: 'd', text: 'A separate cluster for GPU work', whyWrong: 'A second cluster is a heavy answer to a scheduling constraint that a taint expresses in two lines.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Move to the Storage Write API and use a stream with offsets for exactly-once semantics' },
        { id: 'b', text: 'Add insertId values to deduplicate', whyWrong: 'Legacy best-effort deduplication is time-bounded and not a guarantee. It is the mechanism that is already failing them.' },
        { id: 'c', text: 'Batch the rows into hourly load jobs instead', whyWrong: 'Load jobs are cheap and reliable but give up the freshness that motivated streaming in the first place. Only right if nobody needs sub-hour data.' },
        { id: 'd', text: 'Deduplicate downstream with a windowed ROW_NUMBER query', whyWrong: 'You pay to store and scan the duplicates forever, and every consumer must remember to apply the pattern.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Materialize the dashboard’s aggregations, with materialized views or a scheduled summary table, so each refresh reads a small precomputed result' },
        { id: 'b', text: 'Buy a larger slot reservation', whyWrong: 'It makes the same wasteful work finish faster at a higher committed cost. The redundant scanning is untouched.' },
        { id: 'c', text: 'Rely on BigQuery result caching', whyWrong: 'Caching only hits on identical queries, and dashboards parameterize by user, filter and time window, so most refreshes miss.' },
        { id: 'd', text: 'Cluster the base tables more aggressively', whyWrong: 'Clustering helps selective filters. A dashboard aggregating broad ranges still scans broadly.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A policy tag on that column from a taxonomy, granting the fraud team Fine-Grained Reader and leaving analysts without it' },
        { id: 'b', text: 'A view that omits the column, with analysts granted only on the view', whyWrong: 'Workable, but it multiplies views for every combination and the base table is still one accidental grant away from exposure.' },
        { id: 'c', text: 'A second copy of the table with the column removed, refreshed nightly', whyWrong: 'Two copies drift, double the storage, and the sensitive column now exists in one more place for the auditor to track.' },
        { id: 'd', text: 'A row access policy filtering the sensitive records', whyWrong: 'Row policies filter rows, not columns. Every analyst still needs the full record, minus one field.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Cross-location joins are not supported: replicate one dataset into the other location, deliberately, with the residency implications reviewed' },
        { id: 'b', text: 'Set the job location explicitly to US and rerun', whyWrong: 'The job location must match the data. Naming a location does not relocate the EU dataset.' },
        { id: 'c', text: 'Create a federated external table pointing at the other region', whyWrong: 'External table location rules follow the same constraint. It does not create a cross-location join path.' },
        { id: 'd', text: 'Grant the job’s service account access in both projects', whyWrong: 'This is a location constraint, not a permission one. Full access in both places still fails.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Use a transactional store such as Cloud SQL, AlloyDB, Spanner or Firestore for the serving path, and stream into BigQuery for analytics' },
        { id: 'b', text: 'It works if they cluster on user_id', whyWrong: 'Clustering reduces bytes scanned. It does not turn an analytical engine into a low-latency point-lookup store.' },
        { id: 'c', text: 'It works if they buy a slot reservation', whyWrong: 'Reservations fix concurrency and cost predictability, not the per-query latency floor of an analytical engine.' },
        { id: 'd', text: 'It works if updates are batched hourly', whyWrong: 'Batching the writes does not fix the per-request reads, and the application would be serving hour-old profiles.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  // ── AlloyDB and Cloud SQL ────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'AlloyDB with the columnar engine for the scan-heavy queries and a read pool to keep them off the primary' },
        { id: 'b', text: 'A Cloud SQL read replica for the reports', whyWrong: 'It removes the load from the primary but runs the same row-oriented plans, so the reports are no faster.' },
        { id: 'c', text: 'Nightly export into BigQuery', whyWrong: 'A good long-term destination, but it is the second data platform the customer said they were not ready for, and it costs freshness.' },
        { id: 'd', text: 'Add covering indexes for each report query', whyWrong: 'It helps a few queries and taxes every write. Ad hoc reporting outruns index design quickly.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Put a connection pooler between the application and the database so instance count stops driving backend count' },
        { id: 'b', text: 'Raise max_connections on the instance', whyWrong: 'Each connection is a backend process with its own memory. Raising the ceiling moves the failure from rejections to memory pressure.' },
        { id: 'c', text: 'Lower Cloud Run maximum instances', whyWrong: 'This caps the database problem by capping the application’s ability to serve traffic, which is not a fix.' },
        { id: 'd', text: 'Add a read replica and send half the traffic there', whyWrong: 'Read traffic can move, but writes and connection count from 200 instances still land on the primary.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Failover promotes the standby in tens of seconds and drops existing connections, so the application still needs reconnect and retry logic' },
        { id: 'b', text: 'Correct, because the standby shares the same connection endpoint', whyWrong: 'A stable endpoint means clients do not need to change addresses. It does not keep the TCP sessions alive across a promotion.' },
        { id: 'c', text: 'Correct, provided they use the language connector', whyWrong: 'Connectors handle auth and encryption to the instance. They do not preserve in-flight transactions through a failover.' },
        { id: 'd', text: 'Wrong, because failover is manual and requires a support case', whyWrong: 'Regional HA failover is automatic. Overstating the pain is as unhelpful as understating it.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'When the child count per parent is unbounded, or the child is queried independently of the parent as often as with it' },
        { id: 'b', text: 'When the tables are large, since interleaving is only for small tables', whyWrong: 'Size is not the criterion. Locality of access is, and interleaving is used on very large tables routinely.' },
        { id: 'c', text: 'When the child needs its own secondary indexes', whyWrong: 'Interleaved tables can carry their own indexes, including interleaved indexes.' },
        { id: 'd', text: 'When the parent and child are written by different services', whyWrong: 'Write ownership is an organizational concern. The physical layout question is about how the rows are read together.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Run them as read-only transactions with exact or bounded staleness, so they are served without locks and can be answered by a nearby replica' },
        { id: 'b', text: 'Move the reports to a read replica in another region', whyWrong: 'Spanner replicas are not addressable as a separate read endpoint the way a Postgres replica is. Staleness is how you choose where a read may be served.' },
        { id: 'c', text: 'Add processing units to absorb the reporting load', whyWrong: 'It buys headroom without removing the lock and coordination cost that strong reads impose on the write path.' },
        { id: 'd', text: 'Export nightly to BigQuery and report from there', whyWrong: 'A fine long-term pattern, but it turns a seconds-fresh requirement into a day-old one to solve a tuning problem.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Keep that workload on Cloud SQL or AlloyDB: it needs none of the horizontal write scale or multi-region consistency Spanner charges for' },
        { id: 'b', text: 'Standardize on Spanner: one engine reduces operational surface', whyWrong: 'Standardization is real value, but paying a distributed database’s floor and rewriting for its key design on every small service is a poor trade.' },
        { id: 'c', text: 'Use Spanner with the smallest possible capacity to keep it cheap', whyWrong: 'Capacity also governs storage headroom and throughput, so the minimum is not a free tier and it will need to grow.' },
        { id: 'd', text: 'Use Spanner but keep a Cloud SQL replica for cost-sensitive reads', whyWrong: 'Two engines, two schemas and a replication path between them is strictly more cost and more failure modes.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  // ── Firestore and Bigtable ───────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The composite index exists in dev because someone clicked the link in the error, and was never added to the index definitions that deploy to production' },
        { id: 'b', text: 'Production has more documents than the query planner can handle', whyWrong: 'Firestore query cost is proportional to results returned, not collection size. Volume does not cause this failure.' },
        { id: 'c', text: 'Security rules block the ordered read in production', whyWrong: 'A rules denial returns a permission error, not a missing-index error, and rules cannot evaluate ordering.' },
        { id: 'd', text: 'The production database is in a different mode', whyWrong: 'Mode is fixed at creation and would break far more than one query. It is not a per-query symptom.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Shard the counter across several documents, increment a random shard, and sum the shards on read' },
        { id: 'b', text: 'Wrap the increment in a transaction with retries', whyWrong: 'It is already effectively a transaction on that document. Retrying harder against a per-document write ceiling makes the contention worse.' },
        { id: 'c', text: 'Move the counter into a subcollection', whyWrong: 'The write still targets one document. Where it sits in the hierarchy does not change its throughput.' },
        { id: 'd', text: 'Add a composite index on the counter field', whyWrong: 'Indexes serve queries. They do nothing for write throughput, and extra indexes make writes slower.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The Admin SDK bypasses security rules, so the backend’s authorization must be enforced in its own code and scoped by IAM' },
        { id: 'b', text: 'Rules apply to all clients including the Admin SDK', whyWrong: 'They do not. Privileged server credentials are exempt, which is what makes admin operations possible at all.' },
        { id: 'c', text: 'Rules apply unless the backend explicitly disables them per request', whyWrong: 'There is no per-request toggle. The exemption follows the credential type.' },
        { id: 'd', text: 'Rules apply only to writes, not reads', whyWrong: 'Rules govern both. The dividing line is the credential, not the operation.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Bigtable, with a row key of device id plus reversed or bucketed timestamp so recent ranges scan efficiently' },
        { id: 'b', text: 'Firestore, with one document per reading', whyWrong: 'Firestore is not built for that sustained write rate, and a document per reading makes per-device range scans expensive.' },
        { id: 'c', text: 'Spanner, with the device id as the leading key', whyWrong: 'It would work, but you are paying for global strong consistency that telemetry does not need, at a much higher cost per write.' },
        { id: 'd', text: 'BigQuery streaming inserts as the serving store', whyWrong: 'Fine as the analytics destination, wrong as the low-latency lookup path the application queries per device.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Use an app profile with single-cluster routing for that workload: multi-cluster routing gives availability at the cost of read-your-writes' },
        { id: 'b', text: 'Add a short retry loop until the value matches', whyWrong: 'It masks a consistency model with a timing assumption, and the loop fails whenever replication lag exceeds your patience.' },
        { id: 'c', text: 'Enable strong consistency on the replication policy', whyWrong: 'There is no such setting. Cross-cluster replication is eventually consistent, and routing is the lever you have.' },
        { id: 'd', text: 'Move the read-modify-write into a multi-row transaction', whyWrong: 'Bigtable atomicity is per row. Widening the operation does not exist as an option and would not address routing.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Keep them: the guarantee covers redelivery of an acked message, not a crash between performing the side effect and acking' },
        { id: 'b', text: 'Agree, since exactly-once means the handler runs exactly once', whyWrong: 'It bounds delivery, not execution. Any failure after the effect and before the ack still replays the effect.' },
        { id: 'c', text: 'Agree, provided they also enable ordering', whyWrong: 'Ordering constrains sequence within a key. It does not remove duplicate execution.' },
        { id: 'd', text: 'Keep them only for cross-region topics', whyWrong: 'The gap is between side effect and ack, which exists in every region equally.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Ack quickly and hand the long work to a job or workflow, tracking its state outside the subscription' },
        { id: 'b', text: 'Set the ack deadline to its maximum and rely on lease extension', whyWrong: 'It buys time and hides the shape problem. A single slow message still risks expiry, and the subscription now holds work for half an hour.' },
        { id: 'c', text: 'Move the subscription to exactly-once delivery', whyWrong: 'Exactly-once does not stop redelivery of a message that was never acked. The deadline still expires.' },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Use a BigQuery subscription with a topic schema and delete the Dataflow job, keeping Dataflow for transformations that actually need it' },
        { id: 'b', text: 'Keep Dataflow but move it to a smaller worker type', whyWrong: 'You are still paying for and operating always-on workers to do work the subscription performs for free.' },
        { id: 'c', text: 'Replace it with a Cloud Run push subscriber that inserts rows', whyWrong: 'Less infrastructure than Dataflow, but you own retry, batching and dead-lettering code that the managed subscription already handles.' },
        { id: 'd', text: 'Write to Cloud Storage and load hourly instead', whyWrong: 'Cheap, but it converts a streaming requirement into an hourly one without anyone asking.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Whether the subscription was configured in advance to retain acked messages for long enough, since seek can only replay what is still retained' },
        { id: 'b', text: 'Whether the subscription uses pull rather than push', whyWrong: 'Seek works for both. Delivery mode is unrelated to what is retained.' },
        { id: 'c', text: 'Whether the topic has a schema attached', whyWrong: 'A schema validates message structure at publish. It has nothing to do with retention or replay.' },
        { id: 'd', text: 'Whether a dead-letter topic was configured', whyWrong: 'A dead-letter topic collects messages that failed repeatedly. Successfully processed messages never go there.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  // ── Dataflow and Dataproc ────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Move them to Dataproc or Serverless for Apache Spark first, then move individual jobs to Dataflow only where streaming semantics justify the rewrite' },
        { id: 'b', text: 'Rewrite in Beam so everything lands on one runner', whyWrong: 'It couples the migration to a rewrite of 200 tested jobs, which delays every benefit and multiplies the risk of the cutover.' },
        { id: 'c', text: 'Convert the jobs into BigQuery SQL', whyWrong: 'Right for some jobs and a bad fit for others. Deciding it wholesale before profiling the workloads is the same mistake in a different direction.' },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A hot key: all values for one key go to one worker, so combine before the shuffle or salt the key and aggregate in two stages' },
        { id: 'b', text: 'The workers are undersized and need more memory', whyWrong: 'One saturated worker among many idle ones is a distribution problem. A bigger machine raises the ceiling on the same single thread.' },
        { id: 'c', text: 'Streaming Engine is disabled', whyWrong: 'Streaming Engine moves shuffle state off the workers and helps in general, but it does not redistribute a single key across workers.' },
        { id: 'd', text: 'The watermark is advancing too slowly because of late data', whyWrong: 'Late data delays window firing. It does not produce a single hot worker while the rest sit idle.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Match the architecture to the freshness requirement: a scheduled batch job or a direct BigQuery subscription instead of always-on workers' },
        { id: 'b', text: 'Reduce the job to a single small worker', whyWrong: 'It trims the bill but keeps paying continuously for a pipeline whose output is consumed once a day.' },
        { id: 'c', text: 'Enable autoscaling so workers scale to zero when idle', whyWrong: 'Streaming Dataflow autoscaling has a floor of at least one worker. It does not scale to zero between messages.' },
        { id: 'd', text: 'Move the job to Spot workers', whyWrong: 'Preemption on a streaming job means repeated state recovery and lag spikes, for a discount on a cost you should not be paying at all.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  // ── Dataplex and governance ──────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Sensitive Data Protection discovery across the estate, then use the profiles to drive policy tags and access decisions' },
        { id: 'b', text: 'Ask each data owner to complete a classification questionnaire', whyWrong: 'It is slow, incomplete and stale on arrival, because owners describe the schema they intended rather than the data that landed.' },
        { id: 'c', text: 'Grep column names for patterns like email and ssn', whyWrong: 'It misses everything badly named, which is most of it, and flags columns that only look sensitive.' },
        { id: 'd', text: 'Enable Data Access audit logs and infer sensitivity from who reads what', whyWrong: 'Access patterns tell you what is popular, not what is sensitive, and you would need to wait for months of logs.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Dataplex data quality scans defined as rules on each asset, scheduled, with results published as a signal consumers and alerting can read' },
        { id: 'b', text: 'A set of scheduled queries that raise errors when assertions fail', whyWrong: 'Same assertions, no shared reporting surface, and a failing scheduled query is invisible to the people consuming the table.' },
        { id: 'c', text: 'A dbt test suite run in CI', whyWrong: 'Good for build-time checks on transformations, but it does not observe the production table between builds, which is where freshness fails.' },
        { id: 'd', text: 'A Looker alert on each dashboard', whyWrong: 'That detects quality problems only where a dashboard happens to surface them, and only for tables someone charted.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  // ── Cloud KMS and CMEK ───────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Secret Manager, which gives versioning, IAM per secret, audit logging and a rotation path without any ciphertext in the repository' },
        { id: 'b', text: 'Keep the pattern but move the ciphertext to a Cloud Storage bucket', whyWrong: 'It relocates the blob without adding versioning, per-secret access control or rotation, which is what they are missing.' },
        { id: 'c', text: 'Use a KMS key per secret so access can be controlled per key', whyWrong: 'Key sprawl to emulate per-secret IAM, while still owning the storage, versioning and rotation yourself.' },
        { id: 'd', text: 'Inject the values as environment variables at deploy time from CI', whyWrong: 'Then the CI system becomes the untracked secret store, and the values appear in deploy logs and revision history.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Key location has to be compatible with the resource location, so a multi-region dataset needs a key in a matching multi-region' },
        { id: 'b', text: 'BigQuery CMEK requires an HSM protection level', whyWrong: 'Both software and HSM keys are usable. Protection level is a separate choice from location.' },
        { id: 'c', text: 'The dataset must be created before the key is attached', whyWrong: 'CMEK can be set at dataset creation or changed later. Ordering is not the constraint here.' },
        { id: 'd', text: 'Cross-region key use requires VPC Service Controls to be disabled', whyWrong: 'Perimeters are unrelated. The location rule applies with or without them.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Cloud EKM with Key Access Justifications, warning that the external key manager becomes a hard availability dependency for the data' },
        { id: 'b', text: 'CMEK with a rotation schedule and Access Transparency logs', whyWrong: 'That gives visibility after the fact. The requirement is the ability to deny in the moment, which needs the key outside Google.' },
        { id: 'c', text: 'HSM-protected keys in Cloud KMS', whyWrong: 'The key material is protected in hardware but still lives inside Google, so the customer cannot refuse a specific operation.' },
        { id: 'd', text: 'Assured Workloads with a data residency constraint', whyWrong: 'Residency controls where data sits and who may support it. It is not a per-request approval mechanism.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Which obligation applies: where data is stored, who may access and operate it, or whether it must survive a loss of provider access' },
        { id: 'b', text: 'Which regions they want their resources created in', whyWrong: 'That answers only the storage-location question, and it presumes the cheapest interpretation before you know the obligation.' },
        { id: 'c', text: 'Whether they need FedRAMP or equivalent', whyWrong: 'Naming a US program to a European customer suggests you have not read the requirement they are working from.' },
        { id: 'd', text: 'Whether they are willing to pay for Assured Workloads', whyWrong: 'Leading with the price of one product before establishing the requirement is how you sell the wrong tier.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Access Approval, which requires them to approve the access request before it proceeds' },
        { id: 'b', text: 'Access Transparency', whyWrong: 'It gives a log of what was accessed and the justification, after the fact. It is evidence, not consent.' },
        { id: 'c', text: 'CMEK on all resources', whyWrong: 'Customer-managed keys change who controls the key, not whether a support access request proceeds.' },
        { id: 'd', text: 'VPC Service Controls around the projects', whyWrong: 'A perimeter governs API access from network contexts and identities. It does not gate provider support operations.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  // ── Security Command Center ──────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The public access prevention org policy constraint, so the configuration is rejected rather than reported' },
        { id: 'b', text: 'A faster SCC notification channel into their SIEM', whyWrong: 'It shortens the window between exposure and awareness. The bucket is still public in the meantime.' },
        { id: 'c', text: 'An SCC custom module for public buckets', whyWrong: 'Another detection, at best duplicating one that already fired. It cannot block the API call.' },
        { id: 'd', text: 'Remove storage.admin from all users', whyWrong: 'Overbroad, and several other roles carry the permission, so it is neither complete nor operable.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Detection sourced from inside the platform, including threat detection over control plane and audit logs and container runtime signals, alongside native asset inventory' },
        { id: 'b', text: 'More complete misconfiguration coverage than any third party can achieve', whyWrong: 'Overclaiming on the one dimension where the tools genuinely overlap is the fastest way to lose credibility in the room.' },
        { id: 'c', text: 'It is required in order to use org policy constraints', whyWrong: 'Org policy is independent of SCC. Tying them together is simply inaccurate.' },
        { id: 'd', text: 'It replaces the need for a SIEM', whyWrong: 'SCC findings usually flow into the SIEM. It is a source, not a substitute.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Whether Data Access audit logs are enabled for the relevant services: they are off by default, and detections that read them cannot fire without them' },
        ...
        { id: 'c', text: 'Whether the findings are being filtered out by mute rules', whyWrong: 'Possible, and easy to check, but it would not explain a total absence across the entire detection class.' },
        { id: 'd', text: 'Whether the SCC service account has permission to read the projects', whyWrong: 'SCC operates at the org level through its own service agent. A broad permission failure would break far more than one detection type.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Each user id becomes its own time series, so cardinality and cost explode and the metric eventually stops accepting new series' },
        { id: 'b', text: 'The metric will lag because label extraction is asynchronous', whyWrong: 'Extraction happens at ingestion. Latency is not the failure mode here.' },
        { id: 'c', text: 'Log-based metrics cannot have labels', whyWrong: 'They can, and labels are the point. The constraint is on how many distinct values they may take.' },
        { id: 'd', text: 'The user id will be redacted automatically as sensitive data', whyWrong: 'No automatic redaction happens. Keeping user identifiers out of metric labels is a decision you have to make.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Multi-window burn-rate alerts on the SLO error budget: a fast window for sharp outages and a slow one for steady degradation' },
        { id: 'b', text: 'Raise the CPU threshold to 90 percent and lengthen the window', whyWrong: 'Fewer false pages and even more missed outages. The signal is still not connected to user experience.' },
        { id: 'c', text: 'Alert on p99 latency exceeding a fixed millisecond value', whyWrong: 'Closer to the user, but a static threshold has no notion of budget, so a brief spike and a week of degradation look identical.' },
        { id: 'd', text: 'Route CPU alerts to a ticket queue instead of a page', whyWrong: 'It stops the noise waking anyone and does nothing about the outage class that was missed.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Trace context is not carried across the message: the publisher must write it into message attributes and the subscriber must restore it' },
        { id: 'b', text: 'The two services are in different projects so traces cannot join', whyWrong: 'Traces can span projects. The break here is the asynchronous hop, not the project boundary.' },
        { id: 'c', text: 'Sampling is dropping the second half of the trace', whyWrong: 'Sampling would drop whole traces at random, not consistently sever them at the same point.' },
        { id: 'd', text: 'Pub/Sub does not support tracing', whyWrong: 'Instrumented clients do participate in tracing. The propagation still has to be carried in attributes.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A spend-based commitment: a smaller discount, but it is not pinned to a region or machine family that is still moving' },
        { id: 'b', text: 'A resource-based commitment for the current region and family, for the deeper discount', whyWrong: 'It is the deeper discount on capacity they may stop using, in a region they may leave, with the commitment still owed.' },
        { id: 'c', text: 'A one-year resource-based commitment they can renew', whyWrong: 'Shorter, but still region and family locked for a year in which the question says both are expected to change.' },
        { id: 'd', text: 'No commitment until the migration finishes', whyWrong: 'Defensible if the timeline is short, but it forgoes a year or more of discount on the baseline that will exist regardless of where it runs.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Labels apply to usage from when they were set, so historical months can only be attributed by project or folder, not by the new labels' },
        { id: 'b', text: 'Reprocess the billing export with the current labels joined by resource id', whyWrong: 'The mapping today does not tell you which team owned a resource six months ago, and resources have been created and deleted since.' },
        { id: 'c', text: 'Request a corrected billing export from support', whyWrong: 'The export reflects what was recorded at the time. There is no backfill of metadata that did not exist.' },
        { id: 'd', text: 'Use billing account credits to reallocate historical spend', whyWrong: 'Credits adjust amounts owed. They are not an attribution mechanism.' },
      ],
      correctId: 'a',
    },
    origin: 'seed',
