...
export const DRILL_RUNTIME: DrillItem[] = [
  // ── Scaling: reading the load ────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Arrivals are bursty: the same rate lands as short bursts, so instantaneous concurrency is far above the mean' },
        { id: 'b', text: 'The load test used too few virtual users', whyWrong: 'Virtual-user count is a property of the harness. The harness reproduced the mean faithfully, which is exactly what made it useless.' },
        { id: 'c', text: 'Production hardware is slower than the test environment', whyWrong: 'That would raise per-request service time everywhere, not produce queueing that appears and clears in seconds.' },
        { id: 'd', text: 'The latency target was set too aggressively', whyWrong: 'Moving the target does not explain why an identical average rate behaves differently in two places.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'About 600: the arrival rate multiplied by how long each request stays open' },
        { id: 'b', text: 'About 20, one per request per second', whyWrong: 'That is the arrival rate. It only equals concurrency when every request completes within a second.' },
        { id: 'c', text: 'About 30, the response duration', whyWrong: 'Duration alone is not a count. Two requests per second at 30 seconds and two hundred per second at 30 seconds are very different systems.' },
        { id: 'd', text: 'It depends on instance count, not on request duration', whyWrong: 'Instance count determines how much concurrency you can serve. It does not determine how much arrives.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    difficulty: 'deep',
    explanation:
      'Requests are only interchangeable if they cost the same. When a small share of traffic carries a hundred times the context, requests per second stops describing the load and you have to plan in the unit the system is actually consumed in.',
    citations: cite('waf'),
    origin: 'seed',
    criticScore: null,
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Tokens processed per second, since that is what serving capacity is actually spent on' },
        { id: 'b', text: 'Requests per second with a safety factor', whyWrong: 'A factor large enough to cover the heavy tail wastes capacity every other minute of the day, and still guesses at the mix.' },
        { id: 'c', text: 'Concurrent users', whyWrong: 'Two users can differ by two orders of magnitude in what they cost you. This is the same averaging mistake one level up.' },
        { id: 'd', text: 'Peak network bandwidth', whyWrong: 'Bandwidth is rarely the binding constraint for text generation; compute time and provider quota are.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Scaling: horizontal ──────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Instances can no longer be drained freely, so every deploy and every scale-in becomes user-visible unless you handle in-flight streams' },
        { id: 'b', text: 'Nothing: affinity is simply how streaming is done', whyWrong: 'It is common, and it converts each instance into a small stateful server. That is a real trade you should make on purpose.' },
        { id: 'c', text: 'Load balancing becomes significantly more expensive', whyWrong: 'Affinity is a routing setting, not a pricing tier. The cost lands in operations, not the bill.' },
        { id: 'd', text: 'You can no longer terminate TLS at the edge', whyWrong: 'Affinity and TLS termination are unrelated; edge termination is exactly where the affinity cookie is usually set.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Move the trigger out of the process: an external scheduler calling one endpoint, or a leader lease so exactly one instance runs it' },
        { id: 'b', text: 'Stagger the schedule differently on each instance', whyWrong: 'Instances are interchangeable and created by the autoscaler. There is no stable identity to attach per-instance configuration to.' },
        { id: 'c', text: 'Make the job idempotent and let all four run', whyWrong: 'Idempotency is worth having and it does not stop four workers doing four times the work and racing on the same rows.' },
        { id: 'd', text: 'Have each instance check whether it has the lowest id and skip if not', whyWrong: 'That is leader election without a lease or fencing, so two instances can both believe they are lowest during a scale event.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Add damping: react quickly upward, slowly downward, and evaluate the scaling signal over a longer trailing window' },
        { id: 'b', text: 'Raise the maximum instance count', whyWrong: 'The problem is oscillation, not a ceiling. A higher ceiling just gives the oscillation more room.' },
        { id: 'c', text: 'Lower the target utilization', whyWrong: 'Runs more instances all the time and does nothing to damp the loop; it can even make it twitchier.' },
        { id: 'd', text: 'Pin a fixed instance count sized for peak', whyWrong: 'A legitimate fallback, and it pays for peak permanently to avoid tuning a control loop.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Acting as crude admission control that bounded concurrent load on every downstream dependency' },
        { id: 'b', text: 'Saving money', whyWrong: 'A side effect people notice, and the outage revealed that its real job was limiting concurrency downstream.' },
        { id: 'c', text: 'Preventing cold starts', whyWrong: 'A maximum has no effect on how cold a new instance is; a minimum is the setting that does.' },
        { id: 'd', text: 'Enforcing the latency objective', whyWrong: 'It did the opposite: capping instances is what produced the shedding they were trying to remove.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Scaling: caching ─────────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Single-flight recomputation per key, so one request rebuilds an entry while the others wait or are served stale' },
        { id: 'b', text: 'A longer TTL', whyWrong: 'Irrelevant to an empty cache, and it makes staleness worse for every hour the cache is working normally.' },
        { id: 'c', text: 'A larger cache', whyWrong: 'Capacity was never the constraint. The cache was empty, not full.' },
        { id: 'd', text: 'Retrying the database calls that failed', whyWrong: 'Adds load to a saturated database. This is the retry storm that turns a slow recovery into an outage.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The cache key omits the identity that scoped the query, so two tenants asking the same thing share one entry' },
        { id: 'b', text: 'The TTL was too long', whyWrong: 'A shorter TTL narrows the window and leaks just as thoroughly inside it.' },
        { id: 'c', text: 'The cache was not encrypted at rest', whyWrong: 'Encryption stops someone reading the cache store. It does not stop the application handing the wrong entry to the wrong user.' },
        { id: 'd', text: 'Two tenants happened to share a user id', whyWrong: 'That would be a separate identity defect. Here the key contained no identity at all.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Near misses: two questions can be close in embedding space and different in the answer they require, so you serve a confident wrong answer' },
        { id: 'b', text: 'It cannot meaningfully reduce cost', whyWrong: 'Cost reduction is exactly what it delivers, which is why teams are willing to take the correctness risk.' },
        { id: 'c', text: 'It requires storing full prompts, which is not allowed', whyWrong: 'You already log or retain prompts in most designs; retention policy is a separate decision from whether the cache is correct.' },
        { id: 'd', text: 'Embedding the query costs more than generating the answer', whyWrong: 'An embedding call is orders of magnitude cheaper than a generation, which is what makes the pattern viable at all.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Scaling: connection pooling ──────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Requests are queueing for a connection from an exhausted pool, so the wait happens before a query is ever sent' },
        { id: 'b', text: 'The database needs a larger instance', whyWrong: 'It is nearly idle. The work is not reaching it, so more capacity there changes nothing.' },
        { id: 'c', text: 'Queries need better indexes', whyWrong: 'Query times did not move, and index problems move exactly that number.' },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Anything relying on session state across statements: session-level advisory locks, temporary tables, LISTEN and NOTIFY, some prepared-statement usage' },
        { id: 'b', text: 'Anything that uses transactions', whyWrong: 'The transaction is the unit the pooler pins a connection to, so transactional code is precisely what keeps working.' },
        { id: 'c', text: 'Read-only queries', whyWrong: 'Single-statement reads are the easiest case for transaction pooling; they neither need nor keep session state.' },
        { id: 'd', text: 'Anything connecting over TLS', whyWrong: 'The pooler terminates and re-establishes TLS on its own. Encryption is orthogonal to pooling mode.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Whether the HTTP client is being constructed per request, so every call pays a fresh TCP and TLS handshake instead of reusing a pooled connection' },
        { id: 'b', text: 'Whether the provider is throttling you', whyWrong: 'Throttling shows up as 429s or as delayed responses, not as a fixed cost before the request is even sent.' },
        { id: 'c', text: 'Whether the prompt is too large to serialize quickly', whyWrong: 'Serializing a prompt is cheap and would scale with prompt size, not appear uniformly on every request.' },
        { id: 'd', text: 'Whether the model is cold', whyWrong: 'Managed endpoints do not have a per-call warm-up, and a model-side delay would appear after the request left you, not before.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Scaling: queueing ────────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A bound on queue age, with shedding past it and an honest wait estimate surfaced to the user' },
        { id: 'b', text: 'More consumers', whyWrong: 'Adds throughput and tells a user sitting behind forty minutes of backlog nothing at all about what is happening.' },
        { id: 'c', text: 'A faster queue technology', whyWrong: 'The queue was never the bottleneck. The consumers were, deliberately, because that is the point of levelling.' },
        { id: 'd', text: 'A longer visibility timeout', whyWrong: 'Governs redelivery of in-flight messages, which is unrelated to how long work waits before being picked up.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A maximum delivery count routing the message to a dead-letter queue, plus an alert when anything lands there' },
        { id: 'b', text: 'Catch the exception and acknowledge the message', whyWrong: 'Unblocks the queue by silently discarding customer data, and nobody finds out until someone asks where their record went.' },
        { id: 'c', text: 'Add more consumer instances', whyWrong: 'Every consumer fails on the same message. You now fail in parallel.' },
        { id: 'd', text: 'Increase the retry backoff', whyWrong: 'Delays the identical failure indefinitely, and slows down everything queued behind it while you wait.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Ordering serializes everything sharing a key, so that tenant’s events now process one at a time regardless of consumer count' },
        { id: 'b', text: 'There is a hard limit on the number of distinct ordering keys', whyWrong: 'Key cardinality is not the issue here; the collapse is concentrated on one key, not spread across many.' },
        { id: 'c', text: 'Ordered delivery disables batching entirely', whyWrong: 'Messages sharing a key can still be delivered in batches. Sequential processing per key is the constraint, not batch size.' },
        { id: 'd', text: 'That tenant’s messages are larger', whyWrong: 'Payload size would slow things proportionally, not produce a collapse that appears the moment ordering is switched on.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Scaling: hot keys and skew ───────────────────────────────────────────
  {
    ...
    payload: {
      kind: 'mcq',
      stem: 'Why is a timestamp or auto-incrementing id a poor primary key for a horizontally partitioned store under heavy write load?',
      choices: [
        { id: 'a', text: 'Every new write targets the same end of the key space, so one partition absorbs all writes while the others sit idle' },
        { id: 'b', text: 'Sequential keys consume more storage', whyWrong: 'Sequential integers are among the most compact and compressible keys you can choose.' },
        { id: 'c', text: 'They prevent efficient range scans', whyWrong: 'They make time-range scans efficient, which is exactly why teams reach for them in the first place.' },
        { id: 'd', text: 'They cannot be indexed by a distributed store', whyWrong: 'They index fine. The distribution of writes is the problem, not indexability.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Replicate the hot item: cache it in front of the store, or keep copies on several replicas so reads spread' },
        { id: 'b', text: 'Re-shard the whole index', whyWrong: 'Rebalancing relocates the hot key. A single key still lives in one place, so the hotspot moves rather than disappearing.' },
        { id: 'c', text: 'Remove the document from the corpus', whyWrong: 'It is retrieved constantly because it is the most useful thing you have.' },
        { id: 'd', text: 'Rate limit queries that touch it', whyWrong: 'Throttles your most valuable content in order to protect the storage layer, which is backwards.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Per-tenant quotas with fair-share scheduling over the shared limit, so no tenant can take more than its share while others wait' },
        { id: 'b', text: 'Raise the global limit', whyWrong: 'The same tenant consumes the larger limit at the same speed. Fairness is not a capacity problem.' },
        { id: 'c', text: 'Ask that tenant to run their batch overnight', whyWrong: 'Depends on a customer’s cooperation for a property the platform should guarantee, and it fails the day they forget.' },
        { id: 'd', text: 'Have starved clients retry with backoff', whyWrong: 'Makes the starved tenants wait politely while the batch keeps winning every race. Backoff without fairness rewards the aggressive caller.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Scaling: N+1 and chatty calls ────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A per-item fetch inside a loop: the fix is one batched call, not a faster individual call' },
        { id: 'b', text: 'The endpoint is too slow at four milliseconds', whyWrong: 'Four milliseconds is healthy. Three hundred of them serialized in one request is the defect.' },
        { id: 'c', text: 'The tracing library is duplicating spans', whyWrong: 'The wall-clock total matches the span count, so the work is genuinely happening.' },
        { id: 'd', text: 'The handler is CPU starved', whyWrong: 'The time is spent waiting on round trips, which is exactly what a CPU profile would fail to explain.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Score the whole candidate set in a single call, or use a reranking model that takes the full set at once' },
        { id: 'b', text: 'Retrieve fewer candidates', whyWrong: 'Buys latency by cutting recall, which is the exact quality you added reranking to recover.' },
        { id: 'c', text: 'Issue the fifty calls in parallel', whyWrong: 'Improves wall clock and still spends fifty calls of quota and cost on every single query.' },
        { id: 'd', text: 'Cache each document’s relevance score', whyWrong: 'Relevance is a function of the query and the document together, so a per-document score does not transfer to the next query.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Expose list and batch operations alongside the single-record ones, so the model can say "these forty" in one call' },
        { id: 'b', text: 'Expose only single-record tools and raise the step limit', whyWrong: 'Every extra step is a round trip, a token cost and another chance for the model to lose the thread.' },
        { id: 'c', text: 'Let the agent write SQL against the database directly', whyWrong: 'Trades a chatty interface for an unbounded one, and now model output is a query running against production data.' },
        { id: 'd', text: 'Return the entire table so no further calls are needed', whyWrong: 'Fills the context window with data the model never asked for and mostly cannot use, which degrades both cost and accuracy.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Scaling: timeouts, retries, breakers ─────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The caller has already given up before your retries finish, so the retries are pure extra load on a dependency that is already failing' },
        { id: 'b', text: 'The retries usually succeed inside the caller’s budget', whyWrong: 'Three attempts at ten seconds is thirty seconds. The arithmetic does not fit in fifteen, which is the whole point.' },
        { id: 'c', text: 'The dependency sheds the surplus retries automatically', whyWrong: 'Nothing in the design gives the dependency admission control. Assuming somebody else has it is how retry storms happen.' },
        { id: 'd', text: 'The caller retries as well, so the work completes eventually', whyWrong: 'Caller-level retries multiply the same wasted load rather than rescuing it. This is precisely the amplification that turns a slowdown into an outage.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Move to half-open after a cooldown, admit a small number of trial requests, and close only if they succeed' },
        { id: 'b', text: 'Close as soon as the cooldown expires', whyWrong: 'Sends full traffic at a dependency that may still be down, which re-trips the breaker and buries the recovery under the load that broke it.' },
        { id: 'c', text: 'Close when the dependency’s health endpoint returns 200', whyWrong: 'Health endpoints frequently stay green while the code path you actually call is broken. Probe the real path.' },
        { id: 'd', text: 'Wait for an operator to close it manually', whyWrong: 'Useful as an override, and a breaker that requires a human at 3am is not doing the job you added it for.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'An idle timeout: fail when no bytes have flowed for N seconds, rather than when total duration exceeds a limit' },
        { id: 'b', text: 'Raise the total timeout to ten minutes', whyWrong: 'Makes room for the longest answer and also lets genuinely hung connections occupy resources for ten minutes.' },
        { id: 'c', text: 'Return the whole answer in a single response at the end', whyWrong: 'Abandons streaming, which is the feature, and makes the timeout exposure worse rather than better.' },
        { id: 'd', text: 'Have the client reconnect and re-ask when it is cut off', whyWrong: 'Pays for the entire generation twice and shows the user their answer restarting from nothing.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Recording which path served each request, so every downstream metric can be split by it' },
        { id: 'b', text: 'A larger evaluation set', whyWrong: 'A better eval would still be averaging over two different systems it has no way to tell apart.' },
        { id: 'c', text: 'Pinning the fallback model version', whyWrong: 'Worth doing for reproducibility, and it still leaves you unable to attribute any given response to a path.' },
        { id: 'd', text: 'Alerting on primary model errors', whyWrong: 'Tells you the fallback fired at some point. It does not tell you which responses it produced.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Scaling: multi-region ────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Accepting writes in both regions means designing for conflicts and replication lag in the application, which is engineering work rather than a configuration setting' },
        { id: 'b', text: 'It roughly doubles the infrastructure bill', whyWrong: 'True and the least interesting cost. The design and testing burden is what actually derails these projects.' },
        { id: 'c', text: 'Active-active is always slower than a single region', whyWrong: 'It is usually faster for reads, since users hit the nearer region. Write coordination is where the cost lands.' },
        { id: 'd', text: 'It requires a global load balancer they may not have access to', whyWrong: 'Global routing is widely available and the easy part. The hard part is what happens to the data.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The data stayed in region A, so each query pays a cross-region round trip, multiplied by the number of queries per page' },
        { id: 'b', text: 'Region B has slower instance types', whyWrong: 'That would slow computation roughly uniformly, not add a penalty proportional to how many round trips a page makes.' },
        { id: 'c', text: 'Caches are cold in region B', whyWrong: 'Contributes for the first few minutes. A sustained four seconds points squarely at where the data lives.' },
        { id: 'd', text: 'DNS has not fully propagated', whyWrong: 'Propagation decides which region a user reaches, not how long a request takes once it arrives there.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'That the pinned model version and any deployed or tuned endpoints exist in that region, with quota already granted' },
        { id: 'b', text: 'That the API credentials work in both regions', whyWrong: 'Usually true and the least likely thing to bite. Regional availability and quota are granted separately from authentication.' },
        { id: 'c', text: 'That latency from the second region is comparable', whyWrong: 'Worth measuring, and an unavailable model is a much harder failure than a slower one.' },
        { id: 'd', text: 'That the region supports streaming responses', whyWrong: 'Streaming is a property of the API surface, not something that varies region by region.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'One repeated prompt hits cached paths everywhere, including provider prompt caching, so it measured the cheapest possible request' },
        { id: 'b', text: 'It ran from the wrong region', whyWrong: 'Adds a roughly constant network cost. That would depress the numbers, not flatter them.' },
        { id: 'c', text: 'It did not run long enough', whyWrong: 'A longer run of unrepresentative traffic produces more confident wrong numbers.' },
        { id: 'd', text: 'It used too few virtual users', whyWrong: 'Concurrency was clearly high enough to report throughput. The traffic mix, not the volume, is what was unrealistic.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The two survivors must absorb fifty percent more each, which puts them well past capacity: you needed to size for N-1' },
        { id: 'b', text: 'Nothing, because autoscaling adds instances', whyWrong: 'Only if quota, connection pools and boot time all cooperate, and none of that is instantaneous during a zone loss.' },
        { id: 'c', text: 'Traffic drops by a third along with the capacity', whyWrong: 'Users are not located in the zone that failed. Demand does not go anywhere.' },
        { id: 'd', text: 'The load balancer queues the difference until capacity returns', whyWrong: 'Queueing fifty percent excess load converts a capacity shortfall into a latency collapse.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The eval run is not a required check with a pass threshold, so nothing stops a merge when the score drops' },
        { id: 'b', text: 'The eval set needs more cases', whyWrong: 'More cases raise sensitivity and change nothing if the result is advisory and everybody merges anyway.' },
        { id: 'c', text: 'The evals should run nightly instead of per change', whyWrong: 'Nightly finds the regression after it shipped and after several other changes landed on top of it.' },
        { id: 'd', text: 'They need a stronger judge model', whyWrong: 'Improves measurement quality. It still does not make the measurement block anything.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Workload identity federation so CI exchanges its own short-lived token for scoped credentials, with a separate identity per environment' },
        { id: 'b', text: 'Rotate the key every thirty days', whyWrong: 'Shortens the exposure window on a secret that should not exist, and rotation of a key nobody wants to touch tends to quietly stop happening.' },
        { id: 'c', text: 'Store the key in a secret manager and inject it at build time', whyWrong: 'Better hygiene around a long-lived credential that is still long-lived and still ends up in the build environment.' },
        { id: 'd', text: 'Restrict the key to specific source IP ranges', whyWrong: 'Hosted CI runners do not have stable egress addresses, and IP restriction is not a substitute for not having a key.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Productionizing: progressive delivery ────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Traffic is split per request rather than bucketed on a stable user or session identifier' },
        { id: 'b', text: 'Ten percent is too small a sample', whyWrong: 'Sample size affects how confident your metrics are, not whether one user sees two different versions in a row.' },
        { id: 'c', text: 'The two versions were built from different commits', whyWrong: 'That is the intent of a canary. The defect is that one user experiences both of them.' },
        { id: 'd', text: 'The load balancer is not using session affinity', whyWrong: 'Close, and affinity to an instance is not the same as assignment to a variant. You want deterministic bucketing, not stickiness to a machine.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Every schema state must work for both the old and the new code simultaneously, because both colors are live against it during the switch' },
        { id: 'b', text: 'Schema changes must run inside the deployment step', whyWrong: 'Coupling schema and code into one step is the practice expand and contract exists to break.' },
        { id: 'c', text: 'They need a separate database per color', whyWrong: 'Two databases means splitting the data, and now the switch is a data migration rather than a routing change.' },
        { id: 'd', text: 'Blue-green cannot be used with a relational database', whyWrong: 'It is used with relational databases constantly. The compatibility discipline is what makes it work.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The number of possible configurations far exceeds what anybody tests, so production runs a combination that has never been exercised' },
        { id: 'b', text: 'Flag evaluation slows down requests', whyWrong: 'Evaluation is a local lookup measured in microseconds. Performance is not what makes flag debt painful.' },
        { id: 'c', text: 'Flags make rollback harder', whyWrong: 'Flags are one of the few genuinely reversible controls you have. Reversibility is the reason to use them.' },
        { id: 'd', text: 'The flag service becomes a single point of failure', whyWrong: 'A real operational concern, addressed with local defaults and caching, and independent of how many flags you have.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Productionizing: rollback ────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'When the new version has already written data in a shape the old version cannot read or would silently discard' },
        { id: 'b', text: 'When the team believes the fix is small', whyWrong: 'Confidence during an incident is unreliable, and a small fix still goes through a pipeline while the outage continues.' },
        { id: 'c', text: 'When rolling back would look bad to the customer', whyWrong: 'Optics are the worst possible input to a recovery decision, and customers respect fast restoration far more than a clean release history.' },
        { id: 'd', text: 'When the previous version also had bugs', whyWrong: 'A known previous state that mostly worked beats an unknown one you are still debugging.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A rollback trigger agreed before the release: which metric, which threshold, and how long you are allowed to investigate first' },
        { id: 'b', text: 'A faster deployment pipeline', whyWrong: 'Helps once the decision is made, and the delay is almost always in deciding, not in deploying.' },
        { id: 'c', text: 'More people on the incident call', whyWrong: 'More participants usually lengthens the debate. Recovery speed comes from a pre-agreed rule, not a quorum.' },
        { id: 'd', text: 'Better logging', whyWrong: 'Valuable for diagnosis afterwards. Mitigation should not wait for you to understand the cause.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Once no deployed code reads or writes it and the window in which you would still roll back to a version that does has passed' },
        { id: 'b', text: 'Immediately after the new code is deployed everywhere', whyWrong: 'The new code being live is not the same as never needing the old code again. You have just removed your rollback path.' },
        { id: 'c', text: 'In the same release as the code change, to keep the change atomic', whyWrong: 'Atomicity across code and schema is exactly what expand and contract gives up on purpose, because the two cannot deploy simultaneously.' },
        { id: 'd', text: 'When the column is no longer in the ORM model', whyWrong: 'The model reflects one codebase. Reporting jobs, replicas and other services frequently still read the column.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'It needed a lock the live workload was holding, then every subsequent query queued behind the waiting migration' },
        { id: 'b', text: 'Production hardware is slower', whyWrong: 'Production hardware is usually larger. Concurrency, not capacity, is what differs.' },
        { id: 'c', text: 'The migration tool version differed', whyWrong: 'Possible and would normally produce an error rather than a four-minute stall of unrelated queries.' },
        { id: 'd', text: 'The table had more rows', whyWrong: 'Often true and would make the migration itself slow. The outage came from what was blocked behind it, not from the row count.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A full re-embed into a separate index, evaluated against the old one before you switch reads over' },
        { id: 'b', text: 'Re-embed new documents only and leave existing vectors in place', whyWrong: 'Mixed vector spaces make distances meaningless, so retrieval silently degrades in a way no error surfaces.' },
        { id: 'c', text: 'Nothing, provided the dimensionality matches', whyWrong: 'Matching dimensions makes the vectors storable together and not comparable. Different models place meaning in different directions.' },
        { id: 'd', text: 'Rebuild the index in place while serving traffic', whyWrong: 'Leaves you serving from a half-migrated index with no way to compare quality or roll back.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Versioned changes with an author, an audit trail and a one-step revert, so a config edit is as recoverable as a deploy' },
        { id: 'b', text: 'Restricting write access to two senior engineers', whyWrong: 'Reduces frequency and not blast radius. A senior engineer with no revert path still cannot undo a mistake.' },
        { id: 'c', text: 'Requiring changes to be made during business hours', whyWrong: 'A scheduling convention. It does not tell you what changed, who changed it, or how to put it back.' },
        { id: 'd', text: 'Validating the value against a schema on write', whyWrong: 'Catches malformed values and not wrong ones, which is the failure that actually causes the incident.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Productionizing: environments ────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Not as a raw copy: mask or synthesize while preserving distributions and edge cases, or staging inherits production’s compliance scope with weaker controls' },
        { id: 'b', text: 'Fine, since staging is internal only', whyWrong: 'Internal access is broader than production access almost everywhere, which is the entire reason this is a finding.' },
        { id: 'c', text: 'Fine if the copy is encrypted at rest', whyWrong: 'Encryption addresses storage theft. It does nothing about every engineer with staging access being able to read the data.' },
        { id: 'd', text: 'Never use production-like data in lower environments', whyWrong: 'Overcorrects into the parity problem: synthetic data that lacks real messiness hides the defects you most want to catch.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Stub the CRM behind an interface for staging and record real interactions for contract tests, so no lower environment can write to production' },
        { id: 'b', text: 'Leave it and rely on the team being careful', whyWrong: 'Carefulness is not a control, and the first automated test run against staging will not be careful.' },
        { id: 'c', text: 'Use a read-only production credential in staging', whyWrong: 'Better and still leaks production data into a lower environment, and it makes any write path untestable.' },
        { id: 'd', text: 'Block staging from reaching the CRM at the network layer', whyWrong: 'Prevents the damage and leaves the integration entirely untested until it runs in production for the first time.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Every behavioral test in staging: prompt and tool-selection behavior is model-specific, so a staging pass no longer predicts production' },
        { id: 'b', text: 'Nothing, as long as the API surface is the same', whyWrong: 'The API being identical is what makes this so easy to do and so misleading. The behavior is the thing under test.' },
        { id: 'c', text: 'Latency measurements only', whyWrong: 'Latency is the least of it. Correctness of tool calls and output format both move between models.' },
        { id: 'd', text: 'Only tests that check exact wording', whyWrong: 'Exact-wording tests are brittle anyway. What breaks is the higher-level behavior: which tool gets called and whether the output parses.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Alert on user-visible symptoms and error budget burn; keep cause-level conditions on dashboards for diagnosis' },
        { id: 'b', text: 'Raise the thresholds on the noisy alerts', whyWrong: 'Tunes the noise down and still leaves you blind to the failure mode nobody wrote a cause alert for.' },
        { id: 'c', text: 'Route low-severity alerts to a chat channel instead', whyWrong: 'Moves the noise. The unalerted outage is still unalerted.' },
        { id: 'd', text: 'Add alerts for the specific failure that caused last month’s outage', whyWrong: 'Fights the last war. The next outage will have a cause nobody enumerated either.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Split the error rate by variant: if the errors are spread evenly across canary and stable traffic, the deploy is not the cause' },
        { id: 'b', text: 'Roll back and see if the errors stop', whyWrong: 'A reasonable mitigation and a slow, ambiguous experiment: if the real cause is transient, the recovery will look like proof.' },
        { id: 'c', text: 'Read the diff of the deployed change', whyWrong: 'Confirms what changed, not whether it explains the symptom, and it consumes the attention of the people who could be measuring.' },
        { id: 'd', text: 'Check whether the same errors happened after previous deploys', whyWrong: 'Historical base rates are useful in the follow-up and too slow to settle the question during the incident.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: '"Add a pre-deploy check that fails the pipeline when the migration and the code that needs it land in the same release", owned, with a date' },
        { id: 'b', text: '"Engineers will review migrations more carefully"', whyWrong: 'An intention, not a change. The same tired person will make the same call under the same pressure.' },
        { id: 'c', text: '"Document the migration process"', whyWrong: 'A document nobody reads at the moment of the mistake changes no outcome. Encode it in a check.' },
        { id: 'd', text: '"Investigate whether this could happen elsewhere"', whyWrong: 'Unbounded and unassignable, so it stays open forever. Turn it into a specific audit with a scope and an owner.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Pick the dependency whose failure path has never actually executed in production, since untested handling is where the surprises live' },
        { id: 'b', text: 'Pick the dependency that fails most often', whyWrong: 'Its failure path runs regularly, which means it is the one piece of degradation logic you already have evidence for.' },
        { id: 'c', text: 'Pick the slowest dependency', whyWrong: 'Slow is a performance property. It says nothing about whether your handling of its failure works.' },
        { id: 'd', text: 'Pick the one the customer is most worried about', whyWrong: 'Worth addressing in the conversation, and customer intuition about which component is fragile is frequently wrong.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Productionizing: cost ────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Whether you recorded tenant, feature and token counts on every call at the time it was made' },
        { id: 'b', text: 'Whether billing export is enabled', whyWrong: 'Gives you accurate totals per project and per SKU, with no idea which tenant produced them.' },
        { id: 'c', text: 'Whether each tenant has a separate project', whyWrong: 'One way to get attribution, and a heavy one that multiplies quota, deployment and operational overhead per customer.' },
        { id: 'd', text: 'Whether you use a single model version', whyWrong: 'Simplifies unit pricing and tells you nothing about who consumed it.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'They optimized cost per token instead of cost per resolved task, and the cheaper model needs more attempts to finish the job' },
        { id: 'b', text: 'They forgot to include the embedding cost', whyWrong: 'Worth including in the model, and it would not usually reverse the direction of a forty percent unit-price reduction.' },
        { id: 'c', text: 'Their token counting is inaccurate', whyWrong: 'Would make the numbers wrong in one direction consistently, not turn a unit-cost win into a total-cost loss.' },
        { id: 'd', text: 'They did not account for the price of the cheaper model rising', whyWrong: 'Provider prices move slowly and downward. Retry behavior moves immediately.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Controls that act without a human: per-run token ceilings, a hard cap on non-production environments, and automatic scale-down of idle capacity' },
        { id: 'b', text: 'A second alert at fifty percent of budget', whyWrong: 'Two notifications with the same failure mode, which is that nobody has to do anything when they arrive.' },
        { id: 'c', text: 'A weekly cost review meeting', whyWrong: 'Useful for trends and far too slow for a runaway loop that spends a month of budget overnight.' },
        { id: 'd', text: 'More granular cost labels', whyWrong: 'Improves attribution after the fact. It does not stop the spend.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The prompt has been tuned to the eval set: it needs a held-out split and cases refreshed from real production failures' },
        { id: 'b', text: 'The eval set is too small', whyWrong: 'Contributes and is not the mechanism. Even a large set becomes a training target once you optimize against it repeatedly.' },
        { id: 'c', text: 'The judge model is too lenient', whyWrong: 'Would inflate the score from the beginning, not create a growing gap between the score and the user experience.' },
        { id: 'd', text: 'Users are measuring something else entirely', whyWrong: 'Possible, and if the eval measured what users care about, twenty rounds of improvement should show up somewhere.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Shadow the candidate on a sampled slice of live requests, discard its output, and compare the two responses offline' },
        { id: 'b', text: 'A one percent canary', whyWrong: 'A good next step, and it does expose users to the candidate, which is the thing you said you wanted to avoid first.' },
        { id: 'c', text: 'A larger offline eval set', whyWrong: 'Still your imagination of the traffic. Shadowing gets you the actual distribution, including the questions nobody would have written down.' },
        { id: 'd', text: 'Internal dogfooding for a week', whyWrong: 'Cheap and unrepresentative: employees ask systematically different questions from customers.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Productionizing: cutover and data migration ──────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The two writes are not atomic, so a crash or a failure between them leaves the stores divergent with nothing recording that it happened' },
        { id: 'b', text: 'It doubles write latency', whyWrong: 'True and usually acceptable for the duration of a migration. Silent divergence is the problem you cannot accept.' },
        { id: 'c', text: 'The new store may have a different schema', whyWrong: 'Expected and handled by the mapping. Schema difference is the reason for the migration, not a flaw in it.' },
        { id: 'd', text: 'It doubles storage cost', whyWrong: 'The intended and temporary cost of running two systems in parallel.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'After the longest business cycle that touches this data has completed on the new system: month-end, quarterly reporting, any annual process' },
        { id: 'b', text: 'Immediately, since two weeks of clean traffic is convincing', whyWrong: 'Two weeks of daily traffic says nothing about the month-end job or the annual reconciliation that has not run yet.' },
        { id: 'c', text: 'After a fixed ninety days', whyWrong: 'An arbitrary number that may be far too short for an annual process and needlessly long for a simple one.' },
        { id: 'd', text: 'Once the team is confident', whyWrong: 'Confidence is not a criterion anybody can check, which is why the decision drifts either way.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Client: streaming UI ─────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Give each chunk a sequence id and buffer the generation server side, so a reconnecting client resumes from its last received id' },
        { id: 'b', text: 'Increase the client-side retry count', whyWrong: 'Retrying the request restarts the generation. The problem is that there is no way to resume one.' },
        { id: 'c', text: 'Buffer the whole answer and send it in one response', whyWrong: 'Removes streaming, so the user stares at nothing for the full generation and a drop still loses everything.' },
        { id: 'd', text: 'Keep the connection alive with heartbeats', whyWrong: 'Helps against idle-timeout proxies and does nothing when the radio actually drops.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A truthful progress signal naming the current step, so the wait reads as work rather than as a hang' },
        { id: 'b', text: 'An indeterminate spinner', whyWrong: 'Indistinguishable from a frozen client, which is why users start pressing the button again.' },
        { id: 'c', text: 'A fake typing animation until real tokens arrive', whyWrong: 'Fabricates activity, and the moment real tokens appear the transition gives away that the first part was theatre.' },
        { id: 'd', text: 'Nothing until the first token', whyWrong: 'Two seconds of an unchanged screen after a click is read as a failed click.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Keep it, mark it visibly as incomplete, and offer retry or continue' },
        { id: 'b', text: 'Discard it and show an error', whyWrong: 'Throws away tokens the user already paid for and content that may have answered the question.' },
        { id: 'c', text: 'Keep it as if it were complete', whyWrong: 'A truncated answer that looks finished is worse than an error, because somebody will act on it.' },
        { id: 'd', text: 'Automatically regenerate from the start', whyWrong: 'Doubles cost and latency without asking, and the second answer will differ from the first, which is disorienting.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Client: tokens in the client ─────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Move it to the platform keystore or keychain, and hold the short-lived access token in memory only' },
        { id: 'b', text: 'Encrypt it before writing it to the same store', whyWrong: 'The key has to live somewhere on the device too, so you have moved the problem rather than solved it.' },
        { id: 'c', text: 'Shorten the refresh token lifetime', whyWrong: 'Reduces the value of a stolen token and does not stop it being readable in the meantime.' },
        { id: 'd', text: 'Obfuscate the storage key name', whyWrong: 'Security by naming convention. Anyone who can read the store can read all of it.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A backend for frontend that holds the tokens and gives the browser only an httpOnly, SameSite session cookie' },
        { id: 'b', text: 'Keep the token in a JavaScript closure instead of localStorage', whyWrong: 'Better than localStorage against a stray script reading storage, and still reachable by any code executing in the page.' },
        { id: 'c', text: 'Store the token in a non-httpOnly cookie', whyWrong: 'A cookie readable by JavaScript is exactly what the reviewer objected to, with cross-site request forgery added.' },
        { id: 'd', text: 'Use a service worker to hold the token', whyWrong: 'Narrows the exposure and is still script-accessible context in the browser, plus it fails whenever the worker is not registered.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'It hides which behavior the user is choosing: continuing from the partial answer and regenerating from scratch have different costs and different results' },
        { id: 'b', text: 'Retry buttons encourage users to hammer a failing service', whyWrong: 'A rate-limited, disabled-while-pending button handles that. The ambiguity is the real defect.' },
        { id: 'c', text: 'Retries should always be automatic', whyWrong: 'Automatic regeneration spends tokens without consent and replaces the user’s partial answer with a different one.' },
        { id: 'd', text: 'The button should be labelled with the error code', whyWrong: 'Error codes belong in diagnostics, not in the primary action label.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A brief "busy right now, retrying shortly" state that retries automatically after the interval, keeping their question queued' },
        { id: 'b', text: 'An error saying the request was rate limited, with a retry button', whyWrong: 'Pushes an eight-second wait you already know about onto the user as a decision, and blames them for load they did not cause.' },
        { id: 'c', text: 'The raw provider error message', whyWrong: 'Exposes a vendor implementation detail that means nothing to the user and looks like your product is broken.' },
        { id: 'd', text: 'A generic failure so they try again later', whyWrong: 'Discards a request you could satisfy in eight seconds, and generic failures are what make people distrust a product.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The pending state has no client-side deadline, so a response that never arrives has no terminal state to move to' },
        { id: 'b', text: 'The server is too slow', whyWrong: 'Even a fast server drops connections occasionally. A client that cannot handle a missing response is broken regardless of server speed.' },
        { id: 'c', text: 'The spinner animation is misleading', whyWrong: 'The animation is honest about waiting. The problem is that the waiting has no end condition.' },
        { id: 'd', text: 'The request should be retried automatically', whyWrong: 'A retry without a deadline produces two requests that can both hang forever.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Keep it in place, mark it as failed, and offer retry or edit so the content is never lost' },
        { id: 'b', text: 'Remove it and show a toast', whyWrong: 'The user watches their text vanish, and a toast disappears before they can act on it.' },
        { id: 'c', text: 'Leave it looking sent and retry in the background indefinitely', whyWrong: 'Shows a false success. The user believes something was delivered that never was.' },
        { id: 'd', text: 'Remove it and restore the text into the input box', whyWrong: 'Better than losing it, and it breaks the conversation position and discards anything typed since.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The server response is being merged by server id, which the optimistic entry does not have, so it is appended instead of replacing it' },
        { id: 'b', text: 'The list is not sorted deterministically', whyWrong: 'Would change the order of items, not create a second copy of one.' },
        { id: 'c', text: 'The create request was sent twice', whyWrong: 'Would produce two records that both persist, not a duplicate that resolves itself on the next refresh.' },
        { id: 'd', text: 'The cache TTL is too long', whyWrong: 'Stale data shows an old value. It does not manufacture an extra row.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Client: offline and flaky networks ───────────────────────────────────
  {
    ...
    payload: {
      kind: 'mcq',
      stem: 'Two devices edited the same record while offline and both are now syncing. What is the weakest acceptable policy?',
      choices: [
        { id: 'a', text: 'Last write wins by client clock, applied silently to the whole record' },
        { id: 'b', text: 'Field-level merge where edits do not overlap, with a surfaced conflict where they do', whyWrong: 'This is a reasonable policy: it preserves non-conflicting work and asks the user only about genuine collisions.' },
        { id: 'c', text: 'Server-side version check that rejects a stale write and asks the client to reconcile', whyWrong: 'Standard optimistic concurrency. It never destroys data without someone deciding to.' },
        { id: 'd', text: 'Keep both versions and let the user choose', whyWrong: 'Clumsy for frequent edits and safe: nothing is lost without a decision.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The cached data, clearly marked with when it was last updated and that you are offline' },
        { id: 'b', text: 'The cached data with no indication', whyWrong: 'Users make decisions on stale data believing it is current, which is the failure mode that erodes trust fastest.' },
        { id: 'c', text: 'An offline error screen', whyWrong: 'Withholds information you already have and that is very likely still correct.' },
        { id: 'd', text: 'A blocking dialog asking whether they want to see cached data', whyWrong: 'Interrupts to ask a question you can answer with a label, every single time they open the screen.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Client: cancellation and abort ───────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The server must detect the disconnect and abort its own upstream request, or generation continues and you pay for it' },
        { id: 'b', text: 'Nothing: closing the connection propagates cancellation automatically', whyWrong: 'A closed socket is a hint your server has to notice and act on. Nothing in the provider call is cancelled by a browser hanging up.' },
        { id: 'c', text: 'The server should finish the generation and cache it in case the user asks again', whyWrong: 'Defensible for cost only if you actually reuse it, and it holds capacity for a speculative benefit.' },
        { id: 'd', text: 'The server should log the cancellation and continue', whyWrong: 'Records the event and keeps burning tokens and concurrency on abandoned work.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Tag each request and ignore any response that does not belong to the current one, in addition to aborting the previous request' },
        { id: 'b', text: 'Disable the input until the first answer completes', whyWrong: 'Fixes the race by removing a feature users want, and it does not help when the user navigates or edits instead.' },
        { id: 'c', text: 'Debounce the submit handler', whyWrong: 'Debouncing addresses rapid duplicate submits, not a response arriving after a newer one has started.' },
        { id: 'd', text: 'Increase the abort timeout', whyWrong: 'Timeouts govern how long you wait, not which response your state accepts.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Client: accessibility ────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Make the region polite and announce at sentence or completion boundaries rather than on every token' },
        { id: 'b', text: 'Remove the live region entirely', whyWrong: 'Then nothing is announced at all and the user has no idea an answer is arriving.' },
        { id: 'c', text: 'Announce only the final answer, with nothing during generation', whyWrong: 'Leaves the user in silence for the whole generation, unable to tell whether anything is happening.' },
        { id: 'd', text: 'Keep it assertive but update every five tokens', whyWrong: 'Assertive still interrupts, so you have made the stutter slower rather than removing it.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A real focusable button in a stable position with an accessible label, reachable without tabbing through the streaming text' },
        { id: 'b', text: 'A keyboard shortcut documented in the help page', whyWrong: 'Undiscoverable at the moment it is needed, and shortcuts collide with assistive technology bindings.' },
        { id: 'c', text: 'The escape key, since that is conventional', whyWrong: 'Escape conventionally dismisses overlays, and users cannot know it also cancels generation unless something says so.' },
        { id: 'd', text: 'A button that appears on hover over the message', whyWrong: 'Hover-only controls are unreachable by keyboard and by touch.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Color alone carries the meaning, so users with color vision deficiency or a bright screen get no signal at all' },
        { id: 'b', text: 'Amber is the wrong color for a warning', whyWrong: 'Color choice is a design preference. Depending on any color as the sole carrier is the accessibility failure.' },
        { id: 'c', text: 'Background tints reduce text contrast', whyWrong: 'A real and separate contrast concern, fixable while the signal remains color-only.' },
        { id: 'd', text: 'Users will not notice a subtle tint', whyWrong: 'Close, and the point is not subtlety: even a vivid tint communicates nothing to someone who cannot distinguish it.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Focus was on the button that got disabled, so the browser dropped focus to the document root with no announcement' },
        { id: 'b', text: 'The page reloaded', whyWrong: 'A reload would lose the form state too, and the described symptom happens without any navigation.' },
        { id: 'c', text: 'The live region moved focus', whyWrong: 'Live regions announce without taking focus. That is the distinction between a live region and a focus change.' },
        { id: 'd', text: 'The screen reader lost the virtual cursor', whyWrong: 'Describes the symptom rather than the cause. The cause is the focused element disappearing from the tab order.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Virtualize the list so only messages near the viewport are mounted, and keep scroll position stable as items are recycled' },
        { id: 'b', text: 'Memoize each message component', whyWrong: 'Prevents unnecessary re-renders of mounted items and still mounts all 800 on open, which is the cost being measured.' },
        { id: 'c', text: 'Load the conversation in pages of fifty', whyWrong: 'Helps initial load and turns scrollback into a series of waits, and the mounted count still grows without bound.' },
        { id: 'd', text: 'Compress the message payloads', whyWrong: 'Reduces transfer size. The time is spent constructing and laying out DOM, not downloading text.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The initial JavaScript bundle: split out the markdown renderer, syntax highlighter and icon set so they load only when needed' },
        { id: 'b', text: 'The API response time', whyWrong: 'The screen is blank before any API call matters. Nothing can render until the bundle has parsed.' },
        { id: 'c', text: 'Image sizes', whyWrong: 'Worth optimizing, and images load after the first paint rather than blocking it.' },
        { id: 'd', text: 'The number of components on the page', whyWrong: 'Component count affects render time in milliseconds, not the seconds spent downloading and parsing code.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Follow the stream only while the user is already at the bottom; if they scroll up, stop following and show a jump-to-latest control' },
        { id: 'b', text: 'Stop auto-scrolling entirely', whyWrong: 'Then the common case, watching your answer arrive, requires constant manual scrolling.' },
        { id: 'c', text: 'Auto-scroll only when the answer completes', whyWrong: 'Produces one large jump at the end, which is more disorienting than continuous following.' },
        { id: 'd', text: 'Scroll smoothly instead of instantly', whyWrong: 'Makes the same unwanted movement prettier while still dragging the user away from what they are reading.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Tabs refresh independently, so one presents a rotated token another already used, and the server treats the reuse as theft and revokes the session' },
        { id: 'b', text: 'Token lifetimes are too short', whyWrong: 'Short lifetimes make refreshes more frequent, which makes the collision more likely. The race is the cause, not the interval.' },
        { id: 'c', text: 'Cookies are not shared between tabs', whyWrong: 'Cookies are shared across tabs of the same origin, which is precisely why the tabs collide.' },
        { id: 'd', text: 'The clock is skewed between client and server', whyWrong: 'Skew causes consistent early or late expiry, not randomness that scales with the number of open tabs.' },
      ],
      correctId: 'a',
    },
  },
