...
export const DRILL_SECID: DrillItem[] = [
  // ── HIPAA ────────────────────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Expert Determination: a qualified expert assesses and documents that re-identification risk is very small for this specific dataset and recipient' },
        { id: 'b', text: 'Use Safe Harbor anyway and add the dates back after transfer', whyWrong: 'Re-attaching the identifiers you removed re-creates PHI. The de-identification has to hold in the dataset that actually leaves the covered entity.' },
        { id: 'c', text: 'Hash the patient identifiers and call the result de-identified', whyWrong: 'A hash of an identifier is still a link back to the individual. Pseudonymization is a security control, not a de-identification route under the Privacy Rule.' },
        { id: 'd', text: 'Keep it as PHI and rely on encryption in transit', whyWrong: 'That does not answer the question asked. Encryption protects PHI, it does not change whether the data is PHI, so the BAA and minimum-necessary obligations still apply.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'You hold a BAA with the model provider covering that specific service, and the service is one their BAA lists' },
        { id: 'b', text: 'Nothing extra: your BAA with the hospital covers your whole supply chain', whyWrong: 'Your BAA obliges you to bind subcontractors. It does not automatically extend protection to a vendor that never signed anything.' },
        { id: 'c', text: 'The model provider publishes a HIPAA compliance page', whyWrong: 'A marketing page is not a contract. What matters is a signed agreement and the enumerated list of services it covers.' },
        { id: 'd', text: 'You strip patient names before sending, which removes the need for an agreement', whyWrong: 'Clinical narrative with names removed is usually still PHI. De-identification has a defined bar and casual name-stripping does not meet it.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'No: the BAA and the vendor configuration must both exclude PHI from training and improvement, and you should be able to show which setting enforces it' },
        { id: 'b', text: 'Yes, but only in aggregate, which is not identifiable', whyWrong: 'Aggregation is not de-identification, and a use not permitted by the BAA is not permitted regardless of how you describe the output.' },
        { id: 'c', text: 'It depends on whether the customer opts out later', whyWrong: 'Opt-out after the fact does not undo processing that already happened. A permitted-use question is settled before the first request, not after.' },
        { id: 'd', text: 'Only the model provider can answer that', whyWrong: 'You are the business associate the hospital contracted with. Passing the question upstream is not an answer you get to give.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── GDPR ─────────────────────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A valid transfer mechanism such as adequacy or standard contractual clauses with a transfer assessment, and an architecture that keeps the data in-region so the transfer never happens' },
        { id: 'b', text: 'Encryption in transit and at rest, which removes the transfer question', whyWrong: 'Encryption is a safeguard, not a transfer mechanism. Data you can decrypt is still personal data you transferred.' },
        { id: 'c', text: 'The customer’s consent to the transfer, collected in the product', whyWrong: 'Consent-based transfers are narrow and fragile, and consent obtained inside a B2B product rarely comes from the individuals whose data it is.' },
        { id: 'd', text: 'Nothing, because the subprocessor stores the data in an EU region', whyWrong: 'Where the servers sit is only part of it. Remote access by staff outside the EEA is itself a transfer, and it is the part teams forget.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'You document data flows, purposes, retention and safeguards early, because the assessment needs design decisions rather than a finished system' },
        { id: 'b', text: 'Nothing: the DPIA is a legal document written after launch', whyWrong: 'An assessment written after launch cannot influence design, which is the entire point of doing it. It also arrives too late to change anything cheaply.' },
        { id: 'c', text: 'You must obtain consent from every affected individual', whyWrong: 'A DPIA is a risk assessment, not a consent mechanism. Consent may not even be the right lawful basis here.' },
        { id: 'd', text: 'You need to store all processing in one region', whyWrong: 'Residency may come up during the assessment but it is not what triggers or satisfies a DPIA.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Do not put erasable personal data into weights: keep personal context in retrieval, which you can delete and reindex, and treat tuning corpora as de-identified' },
        { id: 'b', text: 'Delete the individual’s rows from the training set and note that the model will forget over time', whyWrong: 'Models do not forget. Removing the source row changes nothing about the weights already trained on it.' },
        { id: 'c', text: 'Retrain the base model from scratch on every erasure request', whyWrong: 'Technically true and operationally absurd. Any process that makes one request cost a full training run will not survive the second request.' },
        { id: 'd', text: 'Rely on the model rarely reproducing training data verbatim', whyWrong: 'Low probability of reproduction is not deletion, and it is not a defensible answer to a data subject request.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Training is a new purpose: it needs its own basis and notice, so tag datasets by permitted purpose rather than only by access level' },
        { id: 'b', text: 'It is fine because the company already holds the data lawfully', whyWrong: 'Lawful collection for one purpose does not authorize any later use. Purpose limitation is the specific rule this reasoning walks into.' },
        { id: 'c', text: 'It is fine if the transcripts stay inside the company', whyWrong: 'Purpose limitation is about what you do with data, not where it goes. Internal use is still use.' },
        { id: 'd', text: 'Anonymize by removing the customer name and proceed', whyWrong: 'Support transcripts are full of identifying detail beyond names. Removing one field is not anonymization and does not settle the purpose question anyway.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── SOC 2 ────────────────────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Explain it is an attestation report over controls you defined, then offer the report so they can check scope, period and exceptions themselves' },
        { id: 'b', text: 'Confirm you are certified and send the logo', whyWrong: 'There is no certificate to hold. Repeating the wrong framing sets up a harder conversation when their auditor reads the actual report.' },
        { id: 'c', text: 'Point them at ISO 27001 instead because SOC 2 is weaker', whyWrong: 'They are different instruments, not a strength ranking, and swapping frameworks does not answer the question they asked.' },
        { id: 'd', text: 'Tell them the auditor certifies the product', whyWrong: 'The report covers a service organization’s controls over a period, not a product’s features.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Point to the complementary user entity controls: several of our controls only hold if you enforce SSO and review your own privileged users' },
        { id: 'b', text: 'Yes, access control is entirely in scope on our side', whyWrong: 'No vendor controls who the customer makes an admin in their own tenant. Claiming otherwise creates a gap nobody owns.' },
        { id: 'c', text: 'Access control is out of scope for SOC 2', whyWrong: 'Logical access is core to the security criteria. It is in scope on the vendor side, with a customer-side counterpart.' },
        { id: 'd', text: 'They should run their own penetration test to be sure', whyWrong: 'A deflection. It does not tell them which responsibilities the report assigns to them.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A bridge letter covering the gap, with the caveat that it is a management assertion rather than audited coverage' },
        { id: 'b', text: 'The Type II report, since it is valid for a year', whyWrong: 'The report describes a specific past period. There is no validity window that extends it forward.' },
        { id: 'c', text: 'A new Type I report for the current date', whyWrong: 'A Type I speaks to design at a point in time and is a downgrade in assurance, not a patch for a coverage gap.' },
        { id: 'd', text: 'Your latest vulnerability scan results', whyWrong: 'Scans say nothing about whether the audited controls kept operating. Wrong artifact for the question.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The carve-out method: the provider’s controls are excluded from your scope, so supply their report alongside yours' },
        { id: 'b', text: 'A control gap you need to remediate before the deal closes', whyWrong: 'Excluding a subservice organization is a standard reporting choice, not a missing control. Remediating it would mean building your own data centers.' },
        { id: 'c', text: 'The inclusive method, which is why the controls appear', whyWrong: 'Inclusive means the subservice organization’s controls are tested inside your report. Here they are explicitly assigned elsewhere, which is the opposite.' },
        { id: 'd', text: 'Evidence that your auditor did not finish the engagement', whyWrong: 'Scope decisions are documented deliberately. Reading them as incompleteness misreads the report structure.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── FedRAMP & public sector ──────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'No: you inherit infrastructure controls, but your service needs its own authorization with an agency sponsor' },
        { id: 'b', text: 'Yes, authorization flows through to anything built on the platform', whyWrong: 'Authorization attaches to a specific service offering and boundary. Nothing inherits automatically or the marketplace would list every SaaS in existence.' },
        { id: 'c', text: 'Yes at Low, and you would need extra work only for Moderate', whyWrong: 'There is no automatic Low authorization for tenants. The impact level describes the data, not a free starting tier.' },
        { id: 'd', text: 'No, and inheriting the cloud’s controls does not help your timeline', whyWrong: 'Too pessimistic. Inheritance genuinely removes a large share of control implementation and evidence work.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The agency’s categorization of the information the system will handle, based on the impact of a confidentiality, integrity or availability loss' },
        { id: 'b', text: 'The number of users the system will serve', whyWrong: 'Scale drives capacity planning, not the security categorization. A ten-user system holding sensitive data is not Low.' },
        { id: 'c', text: 'Whether the system is internet-facing', whyWrong: 'Exposure affects which controls are hard, not which baseline applies. The baseline follows the data.' },
        { id: 'd', text: 'Which cloud region the workload runs in', whyWrong: 'Region matters for boundary and residency questions, but it does not set the impact level.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Logs from the system routinely carry federal data and metadata, so it is either inside the boundary, itself authorized, or the logging has to be scrubbed and kept in-boundary' },
        { id: 'b', text: 'Agreed: telemetry is metadata and metadata is out of scope', whyWrong: 'Application logs are full of request payload fragments, identifiers and error context. Treating them as harmless is how data leaves the boundary quietly.' },
        { id: 'c', text: 'It is fine if the vendor signs an NDA', whyWrong: 'Confidentiality agreements do not put a service inside an authorization boundary or provide the control evidence the package needs.' },
        { id: 'd', text: 'It is fine if logs are encrypted in transit', whyWrong: 'Transport security says nothing about where data comes to rest or who administers that system.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'It is likely a significant change: it goes through the change process with the sponsor before deployment, not into the normal release train' },
        { id: 'b', text: 'None: models are configuration, not architecture', whyWrong: 'A different model is a different processing component with different data handling and different behavior. Calling it configuration is how teams ship an unreviewed change into an authorized boundary.' },
        { id: 'c', text: 'It requires a full re-authorization from scratch', whyWrong: 'Overcorrection. The change process exists precisely so systems can evolve without restarting authorization.' },
        { id: 'd', text: 'It only matters if the new model is from a different vendor', whyWrong: 'Same-vendor version changes can alter behavior and data handling too. Vendor identity is not the test.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Defense impact levels build on a FedRAMP baseline and add requirements such as dedicated infrastructure, personnel constraints and network connectivity, so the answer depends on which level and what those add' },
        { id: 'b', text: 'FedRAMP Moderate satisfies defense impact levels', whyWrong: 'It is a starting point, not equivalence. The additional requirements are exactly where the engineering and staffing cost lands.' },
        { id: 'c', text: 'Impact levels are simply the defense name for FedRAMP baselines', whyWrong: 'They are a separate framework with its own requirements, maintained by a different authority.' },
        { id: 'd', text: 'Impact levels only apply to classified systems', whyWrong: 'Several levels cover controlled unclassified information, which is where most defense SaaS work happens.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── EU AI Act ────────────────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'You carry provider duties for the system itself; the customer carries deployer duties such as operating it as intended, assigning human oversight and keeping logs' },
        { id: 'b', text: 'All duties sit with you as the builder', whyWrong: 'Deployers have their own obligations because they control how the system is used in practice, which is where much of the real-world risk lives.' },
        { id: 'c', text: 'All duties sit with the customer because they decide the use case', whyWrong: 'Providers cannot hand off responsibility for the system’s design, documentation and conformity by pointing at the buyer.' },
        { id: 'd', text: 'Duties only attach once the system is used at scale', whyWrong: 'The framework keys on risk category and role, not on user counts.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Rebranding plus substantial modification can pull them into provider obligations, so they need their own documentation and conformity path, not just yours' },
        { id: 'b', text: 'Nothing: white-labeling is a commercial arrangement with no regulatory effect', whyWrong: 'Placing a system on the market under your own name is precisely one of the conditions that shifts the role.' },
        { id: 'c', text: 'They inherit your conformity assessment automatically', whyWrong: 'Conformity attaches to the system as assessed. A substantial modification changes the thing that was assessed.' },
        { id: 'd', text: 'Only the added decision step matters, not the branding', whyWrong: 'Both routes can trigger the shift independently. Treating branding as neutral misses half the exposure.' },
      ],
      correctId: 'a',
    },
  },
