...
export const DRILL_SECID: DrillItem[] = [
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        ...
        { id: 'b', text: 'Nothing until launch, since the DPIA is a legal artifact describing the shipped system', whyWrong: 'An assessment written after launch cannot influence design, which is the entire point of doing it, and it arrives too late to change anything cheaply.' },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        ...
        { id: 'd', text: 'Your most recent vulnerability scan and penetration test results covering the period', whyWrong: 'Scans say nothing about whether the audited controls kept operating. Wrong artifact for the question.' },
      ],
      correctId: 'b',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'No: obligations under the Act attach only to systems in the high-risk tier', whyWrong: 'The transparency tier exists exactly for systems that are not high-risk. Skipping it because the risk tier is low is the common error.' },
        { id: 'b', text: 'Yes: every AI system placed on the EU market needs a conformity assessment', whyWrong: 'Conformity assessment is a high-risk obligation. Applying it to everything overstates the burden and costs you credibility.' },
        { id: 'c', text: 'Yes: users must be told it is AI, and generated content marked' },
        { id: 'd', text: 'Only if the chatbot processes personal data belonging to EU residents', whyWrong: 'That is the data protection question. Transparency duties follow from the interaction itself, not from the data category.' },
      ],
      correctId: 'c',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Nothing: obligations attach only to the deployed system, not the model layer', whyWrong: 'The framework deliberately places duties on the model layer so downstream builders are not reverse-engineering behavior blind.' },
        { id: 'b', text: 'The vendor’s own compliance flows down, so the customer inherits it', whyWrong: 'Upstream documentation is an input to the customer’s obligations, not a substitute for them.' },
        { id: 'c', text: 'The vendor must classify the customer’s use case and its risk tier for them', whyWrong: 'Classification follows the intended purpose the customer defines. The vendor cannot know that and is not responsible for it.' },
        { id: 'd', text: 'Technical documentation and the information downstream providers need' },
      ],
      correctId: 'd',
    },
  },
  ...
  // ── Residency & sovereignty ──────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Personnel-location limits on provider and vendor support, plus approval and logging of access' },
        { id: 'b', text: 'A written statement that all customer data is stored in EU regions only', whyWrong: 'That answers the storage half of the question they already accepted. They are asking about the access half.' },
        { id: 'c', text: 'Customer-managed encryption keys held in the EU key service they nominate', whyWrong: 'Keys constrain decryption at rest. They do not constrain a support engineer working in a session where the data is already decrypted.' },
        { id: 'd', text: 'A contractual commitment that support tickets are answered within four hours', whyWrong: 'Response time is an operational service level and says nothing about who touches the data.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Allow it: DR copies are only read during an actual disaster declaration', whyWrong: 'A replica holds the data continuously, whether or not anyone reads it. Storage is storage.' },
        { id: 'b', text: 'Constrain allowed regions by org policy and rework the recovery objectives' },
        { id: 'c', text: 'Allow it once the replica is encrypted with a customer-managed key in the EU', whyWrong: 'Encryption does not change the location of the data or satisfy a geographic commitment.' },
        { id: 'd', text: 'Drop cross-region DR entirely and accept a single-region recovery posture', whyWrong: 'False choice. In-geography multi-region DR exists; what changes is the cost and possibly the recovery objectives.' },
      ],
      correctId: 'b',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The container registry holding the images your inference service runs from', whyWrong: 'Images are code artifacts. Worth pinning for supply-chain reasons, but they do not carry customer personal data.' },
        { id: 'b', text: 'The CDN edge cache serving the static assets of your web application', whyWrong: 'Static assets are not personal data. Worth checking for completeness, but it is not where the exposure is.' },
        { id: 'c', text: 'Prompt and response capture in application logs, traces and eval datasets' },
        { id: 'd', text: 'The billing export pipeline that aggregates usage by project and region', whyWrong: 'Billing records usage aggregates, not the content of prompts. Lower sensitivity by a wide margin.' },
      ],
      correctId: 'c',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'It is strictly better than provider-managed keys and costs nothing operationally', whyWrong: 'Selling it as free is how you end up in a post-incident review explaining an outage caused by the customer’s own key service.' },
        { id: 'b', text: 'It is equivalent to customer-managed keys held inside the provider’s key service', whyWrong: 'In-provider customer-managed keys still live in the provider’s key service. The externalized model is the one that changes who can technically deny access.' },
        { id: 'c', text: 'It removes the need for region pinning, since data is unreadable without keys', whyWrong: 'Encryption custody and data location are independent controls. Neither substitutes for the other.' },
        { id: 'd', text: 'Real ability to withhold decryption, and a hard availability dependency' },
      ],
      correctId: 'd',
    },
  },
  ...
  // ── PII & de-identification ──────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Tokenization: the surrogate carries no recoverable value, so scope shrinks' },
        { id: 'b', text: 'Field-level encryption, since AES-256 is stronger than any token mapping', whyWrong: 'Strength is not the question. Ciphertext plus keys in the same estate keeps those systems in scope, which is the outcome they asked to avoid.' },
        { id: 'c', text: 'Either one: both render the value unreadable, so the compliance effect matches', whyWrong: 'The scope-reduction effect is exactly what differs, and it is the entire reason tokenization is used in payments.' },
        { id: 'd', text: 'Neither: a salted hash of the card number is the standard payments approach', whyWrong: 'Hashing destroys the ability to recover the value, which breaks the operational uses that motivated keeping it at all.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'They are right as long as the index carries no metadata fields alongside', whyWrong: 'The vectors themselves carry the content signal. Stripping metadata reduces convenience for an attacker, not the underlying exposure.' },
        { id: 'b', text: 'Embeddings can be inverted enough to recover source content, so the index holds personal data' },
        { id: 'c', text: 'They are right, though access should still be restricted as a precaution', whyWrong: 'Framing it as precaution concedes the wrong premise, and it will not survive a reviewer who has read the inversion literature.' },
        { id: 'd', text: 'It matters only when the embedding model is hosted by a third party', whyWrong: 'Where the model runs affects who processes the text, not whether the resulting vectors encode personal data.' },
      ],
      correctId: 'b',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'It is fine: none of those four fields identifies a person on its own', whyWrong: 'Uniqueness comes from the combination. Evaluating fields one at a time is exactly the reasoning error that produces re-identifiable datasets.' },
        { id: 'b', text: 'Encrypt the four columns and leave the remaining analytical fields in clear', whyWrong: 'Encrypting the columns you need for analysis makes them useless. If they are unusable you should have dropped them; if usable, you have not reduced the risk.' },
        { id: 'c', text: 'Those quasi-identifiers combine to single people out, so generalize them' },
        { id: 'd', text: 'Add independent random noise to each row before releasing the dataset', whyWrong: 'Naive per-row noise degrades utility while leaving combination uniqueness largely intact. Meaningful noise-based privacy needs a formal mechanism and a budget.' },
      ],
      correctId: 'c',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Token cost, which is the main reason to keep prompt payloads short', whyWrong: 'Cost is a real benefit and a weak argument in a security review. Lead with exposure and let cost be the bonus.' },
        { id: 'b', text: 'Models perform measurably worse when given structured JSON as input', whyWrong: 'Not true as a general claim, and it is not why the record should be trimmed.' },
        { id: 'c', text: 'Longer prompts raise the probability of a successful prompt injection attack', whyWrong: 'Injection risk comes from untrusted content in the context, not from length as such. Conflating the two muddies both controls.' },
        { id: 'd', text: 'Minimization: fields you never send cannot be logged, cached or leaked' },
      ],
      correctId: 'd',
    },
  },
  ...
  // ── Tenancy ──────────────────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A missing filter leaks silently; a namespace makes others unreachable' },
        { id: 'b', text: 'Metadata filters degrade recall and latency past a few million vectors', whyWrong: 'Filter performance varies and is rarely the deciding factor. Optimizing latency is not why a security reviewer objects.' },
        { id: 'c', text: 'Namespaces let you run a different embedding model per tenant as they upgrade', whyWrong: 'Possible but unrelated. Mixing embedding models across one retrieval surface is usually a problem, not a feature.' },
        { id: 'd', text: 'Tenants can write index metadata, so the tenant id field cannot be trusted', whyWrong: 'Tenants do not write index metadata in a sane design. The risk is your own query code, not tenant tampering.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Nothing: the cache stores only generated answers, never the source documents', whyWrong: 'The answer is derived from the documents and frequently quotes them. Serving it across a tenant boundary is the leak.' },
        { id: 'b', text: 'Similar prompts collide across tenants, so the key needs tenant and auth context' },
        { id: 'c', text: 'Hit rates on a semantic cache stay too low to justify the added complexity', whyWrong: 'Hit rates on common questions are often high, which is exactly why this failure mode gets shipped and then bites.' },
        { id: 'd', text: 'Latency becomes unpredictable, since a similarity search runs before every call', whyWrong: 'A cache reduces latency variance for hits, and the lookup is cheap next to inference. That is not the issue with this design.' },
      ],
      correctId: 'b',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'It isolates their data from other tenants at every layer of the stack', whyWrong: 'The application holds keys for every tenant it serves. A missing tenant predicate in a query is unaffected by which key encrypted the bytes.' },
        { id: 'b', text: 'It is cosmetic, so decline it and offer a shared key with stronger rotation', whyWrong: 'Cryptographic erasure and a clear per-customer boundary are real. Dismissing the ask loses a control you can charge for.' },
        { id: 'c', text: 'Cryptographic deletion at offboarding, not protection from a cross-tenant query bug' },
        { id: 'd', text: 'It converts the pooled deployment into a siloed one for that customer', whyWrong: 'Key separation is one dimension. Compute, cache, index and network all remain shared.' },
      ],
      correctId: 'c',
    },
  },
  ...
  // ── Zero trust ───────────────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Nothing: the proxy authenticates and authorizes every request end to end', whyWrong: 'If the backend accepts unproxied traffic, anyone who can route to it bypasses the control entirely. The proxy is only as good as the backend’s refusal to be reached directly.' },
        { id: 'b', text: 'Keep its own login as a fallback for periods when the proxy is unavailable', whyWrong: 'A fallback login is a permanent bypass with weaker controls, and it is the path an attacker will use.' },
        { id: 'c', text: 'Allowlist the proxy’s published IP ranges and trust any request from them', whyWrong: 'IP-based trust is the assumption zero trust removes, and it discards the per-user identity the proxy just established.' },
        { id: 'd', text: 'Verify the proxy’s signed identity assertion and refuse direct requests' },
      ],
      correctId: 'd',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Just-in-time elevation with a reason, a short expiry and an alert on use' },
        { id: 'b', text: 'Cut the group down to three engineers who retain permanent production admin', whyWrong: 'Fewer standing credentials is better than more, but the failure mode is unchanged and three people become a single point of both risk and availability.' },
        { id: 'c', text: 'Keep the access as it is and add a documented quarterly access review', whyWrong: 'A review confirms the access still exists. It does nothing about the eighty-nine days between reviews.' },
        { id: 'd', text: 'Require an approved change ticket before anyone logs into production', whyWrong: 'Process without enforcement is a convention people abandon during a real incident, which is precisely when it matters.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Issue contractors a separate read-only account with a distinct role mapping', whyWrong: 'Account splitting is coarse and drifts over time. It also does not tie the decision to device posture, which is the actual risk here.' },
        { id: 'b', text: 'Graduated policy: normal use anywhere, export gated on a managed device' },
        { id: 'c', text: 'Block contractors from the assistant and give them a summarized digest instead', whyWrong: 'That refuses the business requirement rather than meeting it, and the work will route around you into a consumer tool.' },
        { id: 'd', text: 'Place the assistant on a network segment contractors cannot route to at all', whyWrong: 'Network segmentation is the perimeter model. It cannot express "this person, this device, this action".' },
      ],
      correctId: 'b',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Agents make far more API calls, so the exposure window lasts much longer', whyWrong: 'Volume affects cost and rate limits. It does not change what an attacker can reach.' },
        { id: 'b', text: 'Agent activity cannot be covered by the same audit logging as a service', whyWrong: 'It can and must be. Logging is a separate control and does not explain the privilege argument.' },
        { id: 'c', text: 'Its next call is chosen from untrusted text, so permissions cap the damage' },
        { id: 'd', text: 'Model provider terms of service require least privilege for agent integrations', whyWrong: 'Contract terms are not the reason. The reason is that instruction and data arrive on the same channel.' },
      ],
      correctId: 'c',
    },
  },
  ...
  // ── Audit & access transparency ──────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The application can write logs but has no delete or modify permission', whyWrong: 'The application not deleting logs is irrelevant if an operator with project access can. The question is about the humans, not the code path.' },
        { id: 'b', text: 'Logs are encrypted at rest with a customer-managed key held in the EU', whyWrong: 'Encryption protects confidentiality. It does nothing to prevent deletion or modification by someone holding the right permissions.' },
        { id: 'c', text: 'Logs are replicated to a second region within seconds of being written', whyWrong: 'Replication protects against regional loss and faithfully copies a deletion. That is durability, not integrity.' },
        { id: 'd', text: 'Logs export to a separately owned project with retention locked against deletion' },
      ],
      correctId: 'd',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Access Approval, with a warning that a pending request lengthens an outage' },
        { id: 'b', text: 'Access Transparency, which is the standard control customers ask for here', whyWrong: 'Transparency gives visibility, not control. If they asked for control, handing them a log answers a different question.' },
        { id: 'c', text: 'VPC Service Controls, which stop provider personnel reaching the data at all', whyWrong: 'A service perimeter constrains data movement across API boundaries. It is not a control over provider personnel actions.' },
        { id: 'd', text: 'Customer-managed keys, which make administrative access technically impossible', whyWrong: 'Overclaim. Keys inside the provider’s key service do not make administrative access impossible, and saying so damages your credibility fast.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Keep everything for seven years with access tightly restricted and reviewed', whyWrong: 'Access control does not answer a minimization obligation, and it makes every future breach seven years deep.' },
        { id: 'b', text: 'Split it: long retention for actor and references, short for raw text' },
        { id: 'c', text: 'Keep everything thirty days and tell compliance the limit is technical', whyWrong: 'It is not technical, and misrepresenting a design choice as a limitation is how you lose the room.' },
        { id: 'd', text: 'Store salted hashes of prompts for the long window and the text for thirty days', whyWrong: 'A hash of free text is unsearchable and uninterpretable later. It satisfies neither side while looking like a solution.' },
      ],
      correctId: 'b',
    },
  },
  ...
  // ── OIDC & OAuth flows ───────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Nothing, provided the signature and the expiry claims both check out cleanly', whyWrong: 'Signature validity says the token is genuine, not that it was issued for this API or authorizes anything.' },
        { id: 'b', text: 'ID tokens are unsigned assertions, so the API cannot verify them at all', whyWrong: 'ID tokens are signed JWTs. The problem is what they mean, not whether they verify.' },
        { id: 'c', text: 'It is audienced to the client and carries no authorization semantics' },
        { id: 'd', text: 'ID token lifetimes are far too short for a server-to-server API call', whyWrong: 'Lifetime is not the issue, and lengthening it would make this worse rather than better.' },
      ],
      correctId: 'c',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Client credentials, since the build machine has no browser to redirect to', whyWrong: 'Client credentials authenticate the application, not the person. You lose the per-user attribution that was the requirement.' },
        { id: 'b', text: 'Resource owner password credentials, prompting for the password in the shell', whyWrong: 'That grant is deprecated for good reasons: it teaches users to type passwords into arbitrary software and defeats multi-factor and federation.' },
        { id: 'c', text: 'Authorization code with PKCE and a loopback redirect on the build machine', whyWrong: 'Reasonable on a developer laptop, but a headless machine has no browser to open and often no loopback the engineer can reach.' },
        { id: 'd', text: 'Device authorization grant: show a code, approve elsewhere, poll for token' },
      ],
      correctId: 'd',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: '`state` binds the callback to the browser request; `nonce` binds the ID token to it' },
        { id: 'b', text: 'They are redundant, and `state` alone is sufficient in modern OIDC flows', whyWrong: 'They cover different attacks. A valid ID token obtained elsewhere can be injected into a flow that only checks state.' },
        { id: 'c', text: '`nonce` prevents cross-site request forgery and `state` prevents token replay', whyWrong: 'The roles are the other way around, which is exactly the confusion worth clearing up.' },
        { id: 'd', text: 'Both are correlation identifiers used to trace a login across service logs', whyWrong: 'Neither is a correlation identifier. Treating them as telemetry is how implementations stop validating them.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Hold tokens in memory only and never write them to localStorage or cookies', whyWrong: 'Better than localStorage and still reachable by any script running in the page, which is what the bank objected to.' },
        { id: 'b', text: 'A backend-for-frontend holding tokens, with an http-only session cookie' },
        { id: 'c', text: 'Encrypt tokens with a per-session key before storing them in localStorage', whyWrong: 'The decryption key has to live in the same page, so anything that can read the ciphertext can read the key.' },
        { id: 'd', text: 'Use sixty-second token lifetimes with silent refresh in a hidden iframe', whyWrong: 'Short lifetimes shrink the window without removing the exposure, and silent refresh keeps a refresh capability in the browser.' },
      ],
      correctId: 'b',
    },
  },
  ...
  // ── SAML & enterprise SSO ────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Ask the customer to notify your team a week before every rotation window', whyWrong: 'You are outsourcing your uptime to somebody else’s change calendar, and the notification will eventually not arrive.' },
        { id: 'b', text: 'Pin the current certificate and ask them to extend its validity to five years', whyWrong: 'Long-lived signing certificates are the opposite of good hygiene, and most enterprise identity providers will not agree to it.' },
        { id: 'c', text: 'Refresh the IdP metadata on a schedule and accept both keys during overlap' },
        { id: 'd', text: 'Skip signature validation for assertions from customers you already trust', whyWrong: 'That removes the only thing making a SAML assertion trustworthy, converting an availability problem into a total authentication bypass.' },
      ],
      correctId: 'c',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Refuse it outright, since unsolicited assertions cannot be made safe at all', whyWrong: 'Absolutism loses the deal over something with well-understood mitigations that most enterprise SaaS supports.' },
        { id: 'b', text: 'Accept it as equivalent to SP-initiated, since the assertion is signed anyway', whyWrong: 'Signing proves origin, not freshness or intent. The missing request binding is the real difference.' },
        { id: 'c', text: 'Support it with a long assertion validity so a stale portal click never fails', whyWrong: 'A long validity window is precisely what makes a captured assertion useful to an attacker.' },
        { id: 'd', text: 'Weaker without request binding, so enforce single use and a tight window' },
      ],
      correctId: 'd',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'SSO only authenticates; deactivation needs a provisioning channel like SCIM' },
        { id: 'b', text: 'The account expires on its own after a defined period of inactivity', whyWrong: 'Inactivity expiry is a slow, unreliable substitute for deprovisioning and leaves any non-interactive credential intact.' },
        { id: 'c', text: 'Your admin should delete the user manually in the product admin console each time', whyWrong: 'A workable stopgap and a process that fails silently at scale, which is the finding they will get at the next audit.' },
        { id: 'd', text: 'SAML single logout deprovisions the account when the IdP session ends', whyWrong: 'Single logout ends sessions. It has no concept of an account lifecycle.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── JWT validation ───────────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'It costs more CPU than hard-coding a single algorithm in the verifier', whyWrong: 'Performance is irrelevant here. The issue is that an attacker controls a security-critical decision.' },
        { id: 'b', text: 'The token chooses its own verification, enabling `none` and key-type confusion' },
        { id: 'c', text: 'Some issuers omit `alg` entirely, so the header parse can throw at runtime', whyWrong: '`alg` is present in practice. Missing-header handling is not the vulnerability.' },
        { id: 'd', text: 'It breaks whenever the issuer rotates its signing key to a new key pair', whyWrong: 'Key rotation is tracked by `kid`, a separate header field and a separate concern from algorithm selection.' },
      ],
      correctId: 'b',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Fetch it on every single request so a rotation is always picked up immediately', whyWrong: 'That puts a network call in the authentication path for every request and makes the identity provider a single point of failure and a rate-limit victim.' },
        { id: 'b', text: 'Fetch once at process start and hold it for the lifetime of the instance', whyWrong: 'The next key rotation causes a total authentication outage until every instance restarts.' },
        { id: 'c', text: 'Cache with periodic refresh, plus one rate-limited refetch on unknown `kid`' },
        { id: 'd', text: 'Embed the public keys in deployment configuration and never fetch at all', whyWrong: 'It works right up to the first rotation, and then recovery means a deployment across every service at once.' },
      ],
      correctId: 'c',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Tokens start being rejected as soon as the issuer rotates its signing keys', whyWrong: 'Key rotation is unrelated to audience. That describes a different bug entirely.' },
        { id: 'b', text: 'No service can determine which user the presented token actually represents', whyWrong: 'The subject claim still identifies the user. The problem is which service the token was meant for.' },
        { id: 'c', text: 'Token size grows because every service must add its own claims to the token', whyWrong: 'Nothing here changes token size, and size is not a security property.' },
        { id: 'd', text: 'A token minted for the weakest service is accepted by the strictest one' },
      ],
      correctId: 'd',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Up to the remaining token lifetime; check high-impact rights live' },
        { id: 'b', text: 'Immediately, because the identity provider updates the issued token in place', whyWrong: 'Issued tokens are immutable. Nothing updates a token already in the client’s hands.' },
        { id: 'c', text: 'Immediately, as long as every request revalidates the token signature', whyWrong: 'Validation checks signature and expiry. It does not re-read the group membership the claim asserts.' },
        { id: 'd', text: 'Never, until the user explicitly signs out and starts a new session', whyWrong: 'Too pessimistic: the token does expire. The window is the remaining lifetime, not indefinite.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── Token exchange & on-behalf-of ────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Impersonation, so downstream services never need to know an agent was involved', whyWrong: 'Hiding the actor is what makes an incident unresolvable, and it removes any ability to apply agent-specific policy downstream.' },
        { id: 'b', text: 'Delegation: the user stays the subject and the agent is recorded as actor' },
        { id: 'c', text: 'Impersonation, because it yields a smaller token with fewer claims to parse', whyWrong: 'Token size is not a design driver here, and trading auditability for bytes is a poor trade.' },
        { id: 'd', text: 'Either one: the terms are interchangeable names for the same exchange', whyWrong: 'They differ in what the issued token records. That difference is the whole reason both are specified.' },
      ],
      correctId: 'b',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Pass the broad token through, since the calendar API ignores unused scopes', whyWrong: 'The calendar service now holds a credential that also opens mail and files. You have handed your blast radius to whichever integration is weakest.' },
        { id: 'b', text: 'Cache the broad token inside the tool so later calls skip the exchange hop', whyWrong: 'That extends both the lifetime and the reach of the most privileged credential in the system.' },
        { id: 'c', text: 'Exchange it for a short-lived token audienced to the calendar API with calendar scopes only' },
        { id: 'd', text: 'Re-prompt the user to sign in again before each individual tool call', whyWrong: 'Unusable, and unnecessary: exchange exists so consent is captured once and narrowed programmatically.' },
      ],
      correctId: 'c',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Network policy limiting which services can reach the token endpoint at all', whyWrong: 'Reachability is not authorization, and any compromised in-network service defeats it. This is the perimeter assumption again.' },
        { id: 'b', text: 'The short lifetime of the exchanged token, which limits any misuse window', whyWrong: 'A short-lived token that should never have been issued is still an unauthorized delegation.' },
        { id: 'c', text: 'Nothing: any authenticated client holding the token may exchange it freely', whyWrong: 'That is the default you must not accept, which is why the specification defines an actor constraint.' },
        { id: 'd', text: 'An authorization server policy naming which actors may act for a subject' },
      ],
      correctId: 'd',
    },
  },
  ...
  // ── Agent identity & delegation ──────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Two: the agent as a workload, and the user whose consent scopes the call' },
        { id: 'b', text: 'One: the user, since the agent is software running on the user’s behalf', whyWrong: 'Then you cannot revoke a misbehaving agent without revoking the user, and no log can tell you which agent build acted.' },
        { id: 'c', text: 'One: the agent, since it is the component actually making the API call', whyWrong: 'Then downstream services authorize a service account rather than a person, and the user’s own permissions stop constraining anything.' },
        { id: 'd', text: 'Three: the agent, the user, and the model provider that serves the inference', whyWrong: 'The model provider is a subprocessor in the data path, not a principal in your authorization decision.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Add a system prompt telling the model to ignore instructions inside documents', whyWrong: 'It raises the bar slightly and is not a control. Instructions and data share one channel, so this is the model policing itself.' },
        { id: 'b', text: 'Contain it: per-call narrow credentials, approval on writes, restricted egress' },
        { id: 'c', text: 'Scan retrieved documents for known injection phrasings before they enter context', whyWrong: 'Signature matching catches yesterday’s phrasings. Useful in depth, worthless as the primary containment.' },
        { id: 'd', text: 'Run the agent under a service account holding the permissions the product needs', whyWrong: 'Product-wide permissions are exactly the over-broad grant that makes an injection worth exploiting. Scope per call, not per product.' },
      ],
      correctId: 'b',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The orchestrator checks the consent record before it selects the send tool', whyWrong: 'A check in the orchestrator is a check the injected plan can route around. Enforcement has to sit at the resource.' },
        { id: 'b', text: 'The system prompt states that the agent may read mail but never send it', whyWrong: 'Instructions are not enforcement, and this is the exact instruction an injection overrides.' },
        { id: 'c', text: 'Consent becomes token scopes, so the send endpoint refuses on the token' },
        { id: 'd', text: 'The mail send tool is simply not registered in this user’s agent deployment', whyWrong: 'Workable as a coarse on and off switch, and it collapses the moment one user consents to send and another does not in the same deployment.' },
      ],
      correctId: 'c',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A long-lived copy of the token captured in the user’s last interactive session', whyWrong: 'Silently repurposing an interactive credential for unattended use exceeds what the user agreed to and gives you no separate revocation handle.' },
        { id: 'b', text: 'A service account holding permissions across all users, since nobody is present', whyWrong: 'That is the confused-deputy design: one compromise reaches every user, and no log shows on whose behalf anything ran.' },
        { id: 'c', text: 'Nothing extra, because the same token flow works whether or not anyone watches', whyWrong: 'Unattended use gives the user no chance to notice or intervene, which is exactly why it needs tighter scope and visibility.' },
        { id: 'd', text: 'An offline grant with narrower scopes, a stated expiry, and its own revocation path' },
      ],
      correctId: 'd',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A registered per-agent identity the authorization server can refuse alone' },
        { id: 'b', text: 'Revoke the refresh tokens of every user whose session that agent touched today', whyWrong: 'It stops the damage by disabling the victims. Users get logged out and the agent is untouched for everyone else.' },
        { id: 'c', text: 'Roll back the deployment and wait for running instances to cycle out', whyWrong: 'Often the right follow-up and far too slow as the containment step, especially with long-running sessions still in flight.' },
        { id: 'd', text: 'Disable the downstream tools that the agent has been calling incorrectly', whyWrong: 'That removes the capability from every well-behaved agent and user too, which is a bigger outage than the incident.' },
      ],
      correctId: 'a',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Nothing, provided the post-filter is correctly implemented and well tested', whyWrong: 'Correct today and one refactor from a leak, with no other layer to catch it. The data already crossed a boundary before the check ran.' },
        { id: 'b', text: 'A confused deputy: push the user context down into the retrieval query' },
        { id: 'c', text: 'It is slower than filtering at the source, since more candidates are ranked', whyWrong: 'Performance is a side effect. The security argument stands even if post-filtering were faster.' },
        { id: 'd', text: 'The service account should be swapped for the agent’s own workload identity', whyWrong: 'Renaming the over-privileged credential does not reduce what it can read. The problem is that the user is absent from the decision.' },
      ],
      correctId: 'b',
    },
  },
  ...
  // ── Sender-constrained tokens ────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A malicious client requesting scopes far broader than it should ever hold', whyWrong: 'Scope policy is decided at the authorization server. Binding says nothing about what was granted.' },
        { id: 'b', text: 'A compromised resource server reading the contents of the tokens it receives', whyWrong: 'The resource server legitimately reads the token. Binding does not encrypt anything from it.' },
        { id: 'c', text: 'Use of a stolen token by anyone who lacks the bound private key' },
        { id: 'd', text: 'A user sharing their password with a colleague to avoid a licensing cost', whyWrong: 'That happens before any token exists and is addressed by authentication policy, not token binding.' },
      ],
      correctId: 'c',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The access token itself lists the endpoints and methods it may be used on', whyWrong: 'Scopes may constrain operations, but the replay defense here comes from the proof binding, not from the token contents.' },
        { id: 'b', text: 'Proofs are encrypted to the resource server, so the attacker cannot read one', whyWrong: 'Proofs are signed, not encrypted. Readability was never the protection.' },
        { id: 'c', text: 'The resource server keeps a list of every access token it has already seen', whyWrong: 'Replay tracking of proof identifiers helps, but the primary reason is the method and URI binding in the signature.' },
        { id: 'd', text: 'The proof signs the method, the URI and a hash of the access token' },
      ],
      correctId: 'd',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Application-layer proofs on mobile, certificate-bound tokens between services' },
        { id: 'b', text: 'Certificate binding everywhere, for one consistent mechanism to operate', whyWrong: 'Provisioning and rotating client certificates on end-user devices is a support burden most teams abandon, and TLS often terminates before your service anyway.' },
        { id: 'c', text: 'Application-layer proofs everywhere, since they work in both topologies', whyWrong: 'Workable, but it discards the mesh identity you already have between services and adds per-request signing where mutual TLS is free.' },
        { id: 'd', text: 'Neither: short-lived bearer tokens give equivalent protection more cheaply', whyWrong: 'Short lifetimes shrink the replay window. They do not stop a token stolen and used within it, which is the threat here.' },
      ],
      correctId: 'a',
    },
  },
  ...
  // ── RBAC, ABAC and ReBAC ─────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'They need a role hierarchy so specific roles inherit from broader ones', whyWrong: 'Inheritance tidies the naming and does not reduce the combinatorics. You still need one leaf role per combination.' },
        { id: 'b', text: 'Request attributes are encoded in role names; move them into policy' },
        { id: 'c', text: 'They should grant permissions straight to users and retire roles entirely', whyWrong: 'Direct grants remove the only abstraction they have and make access review impossible.' },
        { id: 'd', text: 'Role count of that size is a normal consequence of scale and needs no change', whyWrong: 'Four hundred roles cannot be reviewed by anyone, which means nobody knows what any of them grant.' },
      ],
      correctId: 'b',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Post-filtering costs more tokens because more candidates enter the context', whyWrong: 'Cost is a minor side effect. The problem is that unauthorized content reached the model and the ranking is wrong.' },
        { id: 'b', text: 'Vector stores cannot express per-document permission filters at query time', whyWrong: 'Most support metadata filtering, and where they do not you partition. Capability is not the obstacle.' },
        { id: 'c', text: 'Forbidden content already entered context, and the ranking is wrong' },
        { id: 'd', text: 'The model may hallucinate about documents it was never actually given', whyWrong: 'True in general and unrelated: hallucination happens with or without a filtering strategy.' },
      ],
      correctId: 'c',
    },
  },
  ...
  // ── Scopes, audience & least privilege ───────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Allow it, since the authorization server granted that scope deliberately', whyWrong: 'That makes scope requests an escalation path. The authorization server does not know the user’s rights in every downstream system.' },
        { id: 'b', text: 'Allow the write but record a warning in the security log for later review', whyWrong: 'Logging an unauthorized write does not make it authorized. It just documents the breach.' },
        { id: 'c', text: 'Refuse the write and revoke the token, treating the request as an attack', whyWrong: 'Over-broad scopes are usually sloppy client configuration, not attack. Refusing the operation is right; treating every occurrence as an incident is noise.' },
        { id: 'd', text: 'Refuse: scope caps what the client may attempt, user rights still apply' },
      ],
      correctId: 'd',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Request the token for a named target resource so its audience is that API' },
        { id: 'b', text: 'Give each API its own distinct scope string in the authorization server', whyWrong: 'Scopes describe permissions, and a token carrying all six scopes is still presentable to all six unless the audience is constrained.' },
        { id: 'c', text: 'Register a separate OAuth client for the agent against each of the six APIs', whyWrong: 'It multiplies the credential set the agent holds and still issues tokens that no service is bound to reject.' },
        { id: 'd', text: 'Encrypt each token so that only the intended target service can decrypt it', whyWrong: 'Encrypted tokens exist in some designs and are not the standard mechanism here. The audience check is what services actually enforce.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Everything up front, so the user is never interrupted by a prompt later', whyWrong: 'A maximal consent screen has the worst conversion and hands the agent standing access to systems it may never touch.' },
        { id: 'b', text: 'Only what the first session needs, requesting more when a feature is first used' },
        { id: 'c', text: 'Everything except payments, which is the only genuinely sensitive scope', whyWrong: 'Mail and files are extremely sensitive. Singling out payments misreads where the risk actually is.' },
        { id: 'd', text: 'Nothing at first, then request scopes silently in the background as needed', whyWrong: 'Consent the user never sees is not consent, and no authorization server grants new user scopes without them.' },
      ],
      correctId: 'b',
    },
  },
  ...
  // ── Service-to-service auth ──────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A service account key held in a secrets manager and rotated every quarter', whyWrong: 'Better than a key in a repository and still a long-lived secret that can be copied. Rotation shrinks the window rather than removing the credential.' },
        { id: 'b', text: 'A service account key scoped to one project, which caps the damage it can do', whyWrong: 'Scoping helps and does not address the core problem: a bearer secret that exists can be exfiltrated.' },
        { id: 'c', text: 'Workload identity federation, so no downloadable key exists at all' },
        { id: 'd', text: 'An API key restricted by an IP allowlist covering their egress addresses', whyWrong: 'API keys are weaker than service account keys, and IP allowlists break the moment the workload moves or scales.' },
      ],
      correctId: 'c',
    },
  },
  ...
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'CIDR allowlists are far too slow to evaluate at production request volume', whyWrong: 'They are cheap to evaluate. The problem is what they prove, not what they cost.' },
        { id: 'b', text: 'The approach only works over IPv4 and breaks on dual-stack clusters', whyWrong: 'An implementation detail, not the reason the approach is unsound.' },
        { id: 'c', text: 'It needs a static address, and cloud platforms no longer offer those', whyWrong: 'Static addressing is available. The objection stands even when the address never changes.' },
        { id: 'd', text: 'The range covers every pod, so it authorizes the cluster, not a service' },
      ],
      correctId: 'd',
    },
  },
  ...
  // ── Row-level security ───────────────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'A query missing the tenant predicate returns nothing instead of everything' },
        { id: 'b', text: 'The application no longer needs to know which tenant a request belongs to', whyWrong: 'The policy reads the tenant from session context the application must set correctly. It moves the responsibility, it does not remove it.' },
        { id: 'c', text: 'Each tenant’s rows become transparently encrypted under a separate tenant key', whyWrong: 'That is a key management design. Row-level security is a visibility rule, not encryption.' },
        { id: 'd', text: 'Queries run faster because the engine scans a smaller slice of each table', whyWrong: 'Policy predicates usually cost something. Performance is a thing to watch, not a benefit.' },
      ],
      correctId: 'a',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Pooling and row-level security are incompatible, so drop the pooler', whyWrong: 'They coexist fine. The problem is the scope of the setting, not the existence of the pool.' },
        { id: 'b', text: 'A pooled connection can carry the previous tenant; set it per transaction' },
        { id: 'c', text: 'The policy is re-evaluated for every row, which becomes too slow at scale', whyWrong: 'A performance concern with its own remedies, and not the correctness bug that leaks data across tenants.' },
        { id: 'd', text: 'The pooler cannot forward session settings, so the policy never applies at all', whyWrong: 'The dangerous case is the opposite: the setting persists when it should not.' },
      ],
      correctId: 'b',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'The policies were created but never granted to the application role', whyWrong: 'Policies are not granted to roles; they attach to the table and are evaluated for the current role. This describes a mechanism that does not exist.' },
        { id: 'b', text: 'Row-level security applies only to insert, update and delete, not select', whyWrong: 'Policies can cover select as well, and read protection is the main use.' },
        { id: 'c', text: 'The app connects as the table owner, which is exempt unless policies are forced' },
        { id: 'd', text: 'The table lacks a primary key, so the policy cannot identify rows', whyWrong: 'Unrelated. Policy evaluation does not depend on key constraints.' },
      ],
      correctId: 'c',
    },
  },
  ...
  // ── Impersonation & break-glass ──────────────────────────────────────────
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Full access, because support engineers need to fix things for customers', whyWrong: 'Most tickets are diagnostic. Granting write on every session means the risky capability is used routinely and unremarkably.' },
        { id: 'b', text: 'Full access, with a written policy that agents must ask the customer first', whyWrong: 'A policy is not a control. If the capability is there, it will be used when someone is in a hurry.' },
        { id: 'c', text: 'No impersonation: support should ask the customer to send screenshots instead', whyWrong: 'It pushes customers into sending screenshots full of their own sensitive data, which is worse for privacy than a controlled session.' },
        { id: 'd', text: 'Read-only by default, with write a separate approved and alerted escalation' },
      ],
      correctId: 'd',
    },
  },
  {
    ...
    payload: {
      ...
      choices: [
        { id: 'a', text: 'Only the subject is logged; the operator must be recorded as acting principal' },
        { id: 'b', text: 'Support should not have impersonated an account belonging to a colleague on leave', whyWrong: 'A process observation that fixes nothing. The next impersonation will be just as unattributable.' },
        { id: 'c', text: 'The customer should not be able to read their own tenant’s audit log at all', whyWrong: 'Removing their visibility to hide your gap is exactly the wrong direction, and it is a control they are entitled to.' },
        { id: 'd', text: 'Impersonation sessions should be excluded from the customer’s audit trail', whyWrong: 'They must be included. The customer needs to know their account was accessed and by whom.' },
      ],
      correctId: 'a',
    },
  },
